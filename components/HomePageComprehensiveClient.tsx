...
// ── Main Export ───────────────────────────────────────────────
// ── Market Stats Strip — slim authoritative data band ─────────
function MarketStatsStrip({ stats }: { stats: MarketStats }) {
  ...
  const items = [
    { label: 'Active Condos', value: stats.activeCondos, prefix: '', color: '#3b82f6' },
    { label: 'Active Homes', value: stats.activeHomes, prefix: '', color: '#10b981' },
    { label: 'Buildings Tracked', value: stats.buildingsCount, prefix: '', color: '#f59e0b' },
    { label: 'Avg Condo PSF', value: stats.avgPsf, prefix: '$', color: '#a78bfa' },
  ];

  return (
    <section ref={ref} style={{
      padding: '60px 24px',
      background: 'linear-gradient(180deg, #060b18 0%, #0a1024 100%)',
          ...
          {items.map((item, i) => (
            <div key={item.label} style={{
              ...
              <div style={{
                ...
                {item.prefix}{visible ? item.value.toLocaleString() : 0}
              </div>
              <div style={{
                fontSize: 11,
...
// ── Featured Areas Grid — top GTA municipalities, real data ───
function FeaturedAreasGrid({ areas }: { areas: AreaCard[] }) {
  const [visible, setVisible] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  ...
  const parseTrend = (trend: string) => {
    const n = parseFloat(trend.replace('%', '').replace('+', ''));
    return isNaN(n) ? 0 : n;
  };
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{
          ...
          <p style={{
            ...
            Live listing counts and price trends, updated nightly from MLS data.
          </p>
        </div>
        ...
        <div style={{
          ...
          {areas.map((area, i) => {
            const trendValue = parseTrend(area.trend);
            const trendColor = trendValue > 0.5 ? '#10b981' : trendValue < -0.5 ? '#ef4444' : '#f59e0b';
            const trendArrow = trendValue > 0.5 ? '↑' : trendValue < -0.5 ? '↓' : '→';
              ...
              >
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{
                      ...
                      {area.condoCount.toLocaleString()} condos · {area.homeCount.toLocaleString()} homes
                    </div>
                  </div>
                  {area.avgPsf > 0 && (
                    <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {area.buildingCount.toLocaleString()} buildings
                  </div>
                </div>
              </a>
            );
...
export default function HomePageComprehensiveClient({ agent, stats, topAreas, access }: Props) {
  return (
    <div style={{ minHeight: '100vh', background: '#060b18' }}>
      ...
      <FeaturedAreasGrid areas={topAreas} />
      <WalliamFooter />
    </div>
  );
}
