      ...
      <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {/* Team Settings - Only for Managers */}
            {formData.can_create_children && (
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-orange-900 mb-4 flex items-center gap-2">
                  <Users className="w-5 h-5" />Team Branding (Manager Site)
                </h3>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Team Name</label>
                    <input type="text" value={formData.team_name} onChange={(e) => setFormData({...formData, team_name: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="King Shah Real Estate Team" />
                    <p className="text-xs text-gray-500 mt-1">Displayed on your team website header</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Team Tagline</label>
                    <input type="text" value={formData.team_tagline} onChange={(e) => setFormData({...formData, team_tagline: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Your Trusted Condo Experts in Toronto" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Team Logo URL</label>
                    <input type="url" value={formData.team_logo_url} onChange={(e) => setFormData({...formData, team_logo_url: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="https://example.com/logo.png" />
                    {formData.team_logo_url && <img src={formData.team_logo_url} alt="Team Logo Preview" className="mt-2 h-12 object-contain" />}
                  </div>
                </div>
              </div>
            )}
