...
// W-TERRITORY-MASTER P5.2: Building-tier assignments UI.
// Two modes: Tree (drill by geo) and Search (address/name across geos).
...
export default function BuildingsView({ tenantId, actingAgentId }: Props) {
  ...
  // Tree-mode state (driven by /geo-tree response)
  ...
  // Load buildings whenever scope or search changes
  useEffect(() => {
    loadBuildings()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode, areaId, muniId, communityId, searchDebounced])
  ...
  async function loadBuildings() {
    ...
    if (mode === 'tree') {
      ...
      await fetchBuildings({ scope, scope_id: scopeId })
    } else {
      if (searchDebounced.length < 3) return
      await fetchBuildings({ q: searchDebounced })
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="inline-flex rounded-md shadow-sm border border-gray-200 bg-white" role="group">
        <button
          ...
          className={`px-3 py-1.5 text-xs font-medium rounded-l-md flex items-center gap-1.5 ${mode === 'tree' ? 'bg-green-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
        >
          <Building2 className="w-3.5 h-3.5" /> Tree
        </button>
        <button
          type="button"
          onClick={() => setMode('search')}
          className={`px-3 py-1.5 text-xs font-medium rounded-r-md border-l border-gray-200 flex items-center gap-1.5 ${mode === 'search' ? 'bg-green-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
        >
      ...
      {mode === 'tree' && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-1">Municipality (optional)</label>
              <select value={muniId} onChange={e => setMuniId(e.target.value)} disabled={!currentArea} className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded disabled:bg-gray-100">
                <option value="">- all munis -</option>
                {(currentArea?.municipalities || []).map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs text-gray-600 mb-1">Community (optional)</label>
      ...
      {mode === 'search' && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <label className="block text-xs text-gray-600 mb-1">Address or building name</label>
          <input
            type="text"
            value={searchInput}
      ...
      <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
        ...
        {buildingsLoading ? (
          ...
          <div className="p-6 text-center text-gray-500 text-sm">
            {mode === 'tree' ? 'Pick an area to see buildings.' : 'Type 3+ chars to search.'}
          </div>
        ) : (
