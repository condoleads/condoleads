  ...
  return (
    <div className="relative w-full h-full">
      ...
      {photos.length > 1 && (
        <>
          <button
            onClick={goToPrevious} aria-label="Previous photo"
          ...
          <button
            onClick={goToNext} aria-label="Next photo"
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 z-10">
            {photos.map((_, idx) => (
              <button
                ...
                className={`w-4 h-4 rounded-full transition-colors ${idx === currentIndex ? "bg-white" : "bg-white/50"}`}
              />
            ))}
          </div>
        </>
            ...
            {developments.map((development) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                   {development.galleryPhotos && development.galleryPhotos.length > 1 ? (
            ...
            {paginatedBuildings.map((building) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  {/* Assigned Agent - Only on team sites */}
                  {isTeamSite && building.assigned_agent && (
                    <div className="flex items-center gap-2 mb-4 p-2 bg-blue-50 rounded-lg">
                      <div className="w-8 h-8 rounded-full overflow-hidden bg-blue-200 flex-shrink-0">
                        {building.assigned_agent.profile_photo_url ? (
                          <img src={building.assigned_agent.profile_photo_url} alt={building.assigned_agent.full_name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center text-blue-600 font-semibold text-sm">
                            {building.assigned_agent.full_name.charAt(0)}
                          </div>
                        )}
                      </div>
                      <span className="text-sm text-blue-700 font-medium">Agent: {building.assigned_agent.full_name}</span>
                    </div>
                  )}
