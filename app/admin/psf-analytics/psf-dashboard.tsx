...
import BuildingPSFChart from './building-psf-chart';
...
export default function PSFDashboard({ stats, logs }: Props) {
  const [activeTab, setActiveTab] = useState<'overview' | 'sale' | 'lease' | 'buildings' | 'calculate'>('overview');
  ...
  const [buildingTransactions, setBuildingTransactions] = useState<any[]>([]);
  const [loadingBuildings, setLoadingBuildings] = useState(false);

  // Load buildings with PSF data
  const loadBuildings = async (search: string = '') => {
    setLoadingBuildings(true);
    try {
      const res = await fetch('/api/admin/psf-analytics/buildings?search=' + encodeURIComponent(search));
  ...
  // Load transactions for a specific building
  const loadBuildingTransactions = async (buildingId: string) => {
    try {
      const res = await fetch('/api/admin/psf-analytics/buildings/' + buildingId + '/transactions');
      const data = await res.json();
      if (data.success) {
        setBuildingTransactions(data.transactions || []);
      }
    } catch (error) {
      console.error('Failed to load building transactions:', error);
    }
  };
  ...
  const handleBuildingSelect = async (building: any) => {
    ...
    await loadBuildingTransactions(building.id);
  };
  ...
  useEffect(() => {
    ...
    if (activeTab === 'buildings' && buildings.length === 0) {
      loadBuildings();
    }
  }, [activeTab]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b">
        {(['overview', 'sale', 'lease', 'buildings', 'calculate'] as const).map(tab => (
          <button
            key={tab}
                ...
                {loadingBuildings ? (
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-1">
                    {buildings.map((b) => (
                      ...
                      >
                        <p className="font-medium truncate">{b.building_name}</p>
                        <p className="text-xs text-gray-500">
                          Sales: {b.sale_count || 0} | Leases: {b.lease_count || 0}
                        </p>
                      </div>
                    ))}
                    {buildings.length === 0 && !loadingBuildings && (
                      <p className="text-gray-500 text-sm">No buildings with PSF data</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="col-span-2 space-y-4">
              {selectedBuilding ? (
                <>
                  {/* Summary Card */}
                  <div className="bg-white border rounded-lg p-4">
                    <h3 className="font-semibold text-lg mb-3">{selectedBuilding.building_name}</h3>
                    <div className="grid grid-cols-4 gap-4">
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Sale Avg PSF</p>
                        <p className="text-xl font-bold text-blue-600">
                          ${selectedBuilding.sale_avg_psf?.toFixed(0) || 'N/A'}
                        </p>
                      </div>
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Sale Count</p>
                        <p className="text-xl font-bold">{selectedBuilding.sale_count || 0}</p>
                      </div>
                      <div className="bg-green-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Lease Avg PSF</p>
                        <p className="text-xl font-bold text-green-600">
                          ${selectedBuilding.lease_avg_psf?.toFixed(2) || 'N/A'}
                        </p>
                      </div>
                      <div className="bg-green-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Lease Count</p>
                        <p className="text-xl font-bold">{selectedBuilding.lease_count || 0}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* PSF Chart */}
                    {buildingTransactions.length > 0 && (
                      <BuildingPSFChart 
                        transactions={buildingTransactions} 
                    ...
                    {/* Transactions Table */}
                    <div className="bg-white border rounded-lg p-4">
                      <h3 className="font-semibold mb-3">Transaction History ({buildingTransactions.length})</h3>
                    <div className="max-h-96 overflow-y-auto">
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-white border-b">
                          <tr>
                            ...
                            <th className="text-right py-2 px-2">PSF</th>
                            <th className="text-center py-2 px-2">Method</th>
                          </tr>
                        </thead>
                        <tbody>
                          {buildingTransactions.map((t, i) => (
                            <tr key={i} className="border-b hover:bg-gray-50">
                              <td className="py-2 px-2">{t.close_date}</td>
                              ...
                              <td className="text-right py-2 px-2">${t.close_price?.toLocaleString()}</td>
                              <td className="text-right py-2 px-2">{t.sqft}</td>
                              <td className="text-right py-2 px-2 font-medium">${t.psf?.toFixed(2)}</td>
                              <td className="text-center py-2 px-2">
                                <span className={`px-2 py-0.5 rounded text-xs ${
                                  t.sqft_method === 'exact' ? 'bg-green-100 text-green-700' :
                                  t.sqft_method === 'midpoint' ? 'bg-yellow-100 text-yellow-700' :
                ...
                <div className="bg-gray-50 border rounded-lg p-8 text-center text-gray-500">
                  Select a building to view PSF data and transactions
                </div>
              )}
            </div>
          </div>
