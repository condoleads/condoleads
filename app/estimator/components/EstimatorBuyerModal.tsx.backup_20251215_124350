        ...
        <div className="p-6">
          ...
          {!result && !error && (
            <button
              ...
              disabled={loading}
              className={`w-full ${isSale ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-sky-600 hover:bg-sky-700'} disabled:bg-slate-400 text-white py-4 px-8 rounded-xl font-semibold text-lg transition-colors shadow-lg`}
            >
              {loading ? 'Analyzing Market Data...' : `Get ${isSale ? 'Price' : 'Lease'} Estimate`}
            </button>
          )}
