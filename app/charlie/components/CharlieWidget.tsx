...
export default function CharlieWidget({ pageContext }: CharlieWidgetProps = {}) {
  ...
  return (
    <>
      ...
      {state.approvalNotification && !approvalDismissed && (
        <div style={{
          position: 'fixed', bottom: 90, left: '50%',
          transform: 'translateX(-50%)',
          zIndex: 9999, width: 'calc(100% - 40px)', maxWidth: 460,
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: '3px 0 0', color: 'rgba(255,255,255,0.8)', fontSize: 13, lineHeight: 1.5 }}>Your agent approved your request. Your credits have been refreshed.</p>
          </div>
          <button onClick={() => setApprovalDismissed(true)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', fontSize: 18, padding: '0 0 0 4px', lineHeight: 1, flexShrink: 0 }}>✕</button>
        </div>
      )}
