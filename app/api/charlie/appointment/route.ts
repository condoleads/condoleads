...
//
// W-HIERARCHY H3.6 (2026-05-03):
//   - walkHierarchy replaces inline parent_id query (F48 + F66)
//   - getLeadEmailRecipients enforces full 6-layer fan-out (F65 mechanism retired)
//   - lead insert payload now includes area_manager_id + tenant_admin_id (F58)
  ...
  try {
    ...
    if (agentId) {
      ...
      if (agentData) {
        ...
        // Replace direct parent_id query with full hierarchy walk
    ...
    try {
      ...
      if (err instanceof AdminPlatformUnreachable) {
        ...
        // Lead row already saved; soft-fail the chain notification
        recipients = null
      } else {
        throw err
      }
