// app/api/admin-homes/territory/cards/bulk-restore/route.ts
// W-TERRITORY-OPS T1-4 -- bulk restore apa cards.
//
// POST { card_ids: uuid[] }
//   Restores (is_active = true) every card_id provided.
...
// handle_apa_update trigger already writes assignment_granted audit row on the
// OLD.is_active IS NOT TRUE AND NEW.is_active IS TRUE branch (verified
// 2026-05-25 via _w-territory-ops-T1-4-deploy.js Section 2 PASS).
//
// Returns: { ok, restored_count, queued_count }
...
export async function POST(req: NextRequest) {
  ...
  // Verify every card belongs to the tenant + is currently inactive (otherwise restore is a no-op).
  const s = svc()
  ...
  const alreadyActive = rows.filter((r: any) => r.is_active).length
  if (alreadyActive === rows.length) {
    return NextResponse.json({ error: 'all cards already active' }, { status: 409 })
  }
  ...
  await c.connect()
  let result: { restored_count: number; queued_count: number } | null = null
  try {
    await c.query('BEGIN')
    ...
    result = { restored_count: restored, queued_count: queued }
  } catch (e: any) {
