...
async function calculateForType(
  ...
  if (level === 'gta') {
    ...
    // Fetch buildings from our database with full geographic hierarchy
    ...
    console.log(`[PSF ${type}] Processing ${buildings.length} buildings from database`);
    ...
    for (let i = 0; i < buildings.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async (building: any) => {
        try {
          ...
          for (const metrics of periodMetrics) {
            await saveBuildingMetrics(
              tableName,
              building.id,
              building.community_id,
              building.communities.municipality_id,
              building.communities.municipalities.area_id,
              metrics
            );
            results.periods++;
          }
