...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fullData && filteredListings.length > 0) {
      console.log('ENHANCED: Collecting PropertyRooms, Media, and OpenHouses data (PARALLEL)...');
      
      const BATCH_SIZE = 10;
      const totalBatches = Math.ceil(filteredListings.length / BATCH_SIZE);
      
      // Helper function to fetch all enhanced data for one listing
      const fetchEnhancedDataForListing = async (listing: any, index: number) => {
        const listingKey = listing.ListingKey;
        if (!listingKey) return;
        
        // Fetch rooms, media, and open houses in parallel for this listing
        const [roomsResult, mediaResult, openHouseResult] = await Promise.all([
          // Rooms
          fetch(`${baseUrl}PropertyRooms?$filter=${encodeURIComponent(`ListingKey eq '${listingKey}'`)}&$top=50`, {
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
          }).then(r => r.ok ? r.json() : { value: [] }).catch(() => ({ value: [] })),
          
          // Media
          fetch(`${baseUrl}Media?$filter=${encodeURIComponent(`ResourceRecordKey eq '${listingKey}'`)}&$top=500`, {
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
          }).then(r => r.ok ? r.json() : { value: [] }).catch(() => ({ value: [] })),
          
          // Open Houses
          fetch(`${baseUrl}OpenHouse?$filter=${encodeURIComponent(`ListingKey eq '${listingKey}'`)}&$top=20`, {
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
          }).then(r => r.ok ? r.json() : { value: [] }).catch(() => ({ value: [] }))
        ]);
        ...
        if (index < 3) {
          console.log(`Listing ${listingKey}: ${listing.PropertyRooms.length} rooms, ${listing.Media.length} media, ${listing.OpenHouses.length} open houses`);
        }
      };
      ...
      for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        const batch = filteredListings.slice(start, start + BATCH_SIZE);
        
        console.log(`Processing batch ${batchIndex + 1}/${totalBatches} (${batch.length} listings)...`);
        
        await Promise.all(batch.map((listing, idx) => fetchEnhancedDataForListing(listing, start + idx)));
      }
      
      console.log(`ENHANCED: Completed all ${filteredListings.length} listings in ${totalBatches} batches`);
    }
