...
export async function POST(request: NextRequest) {
  ...
  try {
    const { buildingData, listingsChunk, chunkIndex, totalChunks } = await request.json();
    const listingsData = listingsChunk;
    
    // DEBUG: Check if enhanced data exists
    console.log(` First listing has Media: ${listingsData[0]?.Media?.length || 0}`);
    ...
    let building;
    if (chunkIndex === 0) {
      building = await saveBuildingData(buildingData);
    } else {
      // Get existing building for subsequent chunks
      const { data: existingBuilding } = await supabase
        .from('buildings')
        .select('*')
        .eq('slug', buildingData.slug)
        .single();
      
      if (!existingBuilding) {
        throw new Error(`Building not found for chunk ${chunkIndex}`);
      }
...
async function forceCleanBuildingByAddress(streetNumber: string, streetName: string, city: string) {
  // Extract first word of street name for matching
  const streetFirstWord = streetName.split(' ')[0];
  console.log(`ðŸ§¹ Force cleaning buildings by address: ${streetNumber} ${streetFirstWord}%, ${city}`);
  
  // Find ALL buildings matching street_number + street_name (first word) + city
  const { data: existingBuildings } = await supabase
    .from('buildings')
    .select('id, building_name, slug')
  ...
  if (existingBuildings && existingBuildings.length > 0) {
    console.log(`Found ${existingBuildings.length} existing building(s) to clean:`, existingBuildings.map(b => b.building_name));
    
    for (const existingBuilding of existingBuildings) {
      console.log(`Cleaning building: ${existingBuilding.building_name} (${existingBuilding.id})`);
      
      // Get all listing IDs for this building
...
// STEP 3: Save listings with complete DLA mapping
async function saveListingsWithCompleteDLAMapping(buildingId: string, listingsData: any[]) {
  ...
  for (let i = 0; i < listingsData.length; i += batchSize) {
    ...
    const batchRecords = batch.map(listing => mapCompleteDLAFields(listing, buildingId));
    ...
    data?.forEach((savedListing, index) => {
      savedListings.push({
        ...savedListing,
    ...
    console.log(` Batch ${Math.floor(i / batchSize) + 1} saved: ${data?.length || 0} listings`);
  }
  
  return savedListings;
}
// COMPLETE DLA FIELD MAPPING FUNCTION \(ALL 470\+ FIELDS\)

// Validation helper to catch VARCHAR overflows
function validateAndTruncate(value: any, maxLength: number, fieldName: string, listingKey?: string): any {
  if (typeof value === 'string' && value.length > maxLength) {
    console.error(` FIELD OVERFLOW: ${fieldName}`);
    console.error(`   Length: ${value.length} (max: ${maxLength})`);
    console.error(`   Value: "${value}"`);
    if (listingKey) console.error(`   ListingKey: ${listingKey}`);
    ...
    if (originalListing?.PropertyRooms && Array.isArray(originalListing.PropertyRooms)) {
      for (const room of originalListing.PropertyRooms) {
        roomRecords.push({
          ...
          // listing_key removed - doesn't exist in table
...
async function ensureGeographicHierarchy(
  ...
  try {
    ...
    // STEP 2: Ensure Municipality exists under this Area
    ...
    if (!existingMunicipality) {
      ...
      console.log(`  Creating municipality: ${municipalityName} (code: ${code})`);
    ...
    // STEP 3: Ensure Community exists under this Municipality
...
async function linkBuildingToHierarchy(
  buildingId: string, 
  listingsData: any[]
): Promise<void> {
  ...
  const communityId = await ensureGeographicHierarchy(
    ...
    firstListing.CityRegion        // Community: "Waterfront Communities C1"
  );
