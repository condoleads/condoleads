...
export async function POST(request: NextRequest) {
  try {
    ...
    const { buildingId, triggeredBy = 'manual' } = await request.json();
    ...
    if (!building.community_id && proptxListings.length > 0) {
      console.log('ðŸ—º️ Building missing community_id - backfilling geographic hierarchy...');
    ...
    const totalListingsFound = proptxActive.length + proptxInactive.length;
    const totalListingsCreated = (activeResults.added || 0) + (inactiveResults.added || 0);
    const totalMedia = (activeResults.mediaAdded || 0) + (inactiveResults.mediaAdded || 0);
    const totalRooms = (activeResults.roomsAdded || 0) + (inactiveResults.roomsAdded || 0);
    const totalOpenHouses = (activeResults.openHousesAdded || 0) + (inactiveResults.openHousesAdded || 0);
  ...
  if (proptxActive.length > 0) {
    for (const [listingKey, dbListing] of dbMap) {
      if (!proptxMap.has(listingKey)) {
        ...
        results.removed++;
        console.log(`ðŸ—‘️ Removed: ${listingKey} (no longer active)`);
      }
    }
  }

  console.log(`✅ Active Results: +${results.added} ✏️${results.updated} ðŸ—‘️${results.removed} ⏺️${results.unchanged}`);
