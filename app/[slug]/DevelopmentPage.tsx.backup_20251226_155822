...
export async function generateDevelopmentMetadata(development: { id: string; name: string; slug: string }) {
  ...
  // Get building count and listing stats
  ...
  const title = `${development.name} | ${buildingCount} Buildings | ${siteName}`
  const description = `Explore ${development.name} - a premier Toronto development with ${buildingCount} buildings. View available condos for sale and rent, floor plans, amenities, and market insights.`
