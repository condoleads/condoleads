              ...
              {forSaleActive.slice(0, 12).map((listing: any) => (
                <div key={listing.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                  <div className="aspect-video bg-gray-200 relative overflow-hidden group">
                    ...
                    <div className="absolute top-3 left-3"><span className="px-3 py-1 bg-emerald-500 text-white text-sm font-semibold rounded-full">For Sale</span></div>
                    <div className="absolute bottom-3 left-3 text-2xl font-bold text-white">{formatPrice(listing.list_price)}</div>
                  </div>
                  <div className="p-4">
              ...
              {forLeaseActive.slice(0, 12).map((listing: any) => (
                <div key={listing.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                  <div className="aspect-video bg-gray-200 relative overflow-hidden group">
                    <img src={getListingPhoto(listing)} alt={listing.unit_number || 'Unit'} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <div className="absolute top-3 left-3"><span className="px-3 py-1 bg-sky-500 text-white text-sm font-semibold rounded-full">For Rent</span></div>
                    <div className="absolute bottom-3 left-3 text-2xl font-bold text-white">{formatPrice(listing.list_price)}/mo</div>
                  </div>
                  <div className="p-4">
                    <h3 className="font-bold text-gray-900">Unit {listing.unit_number || 'N/A'}</h3>
                    <p className="text-gray-600 text-sm">{listing.bedrooms_total || 0} bed • {listing.bathrooms_total_integer || 0} bath • {listing.living_area_range || 'N/A'} sqft</p>
