...
export default async function CommunityPage({ community }: CommunityPageProps) {
  ...
  const [municipalityResult, initialListingsResult, forSaleCount, forLeaseCount, soldCount, leasedCount, agentResult] = await Promise.all([
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <nav className="text-sm text-gray-500 mb-4">
          {area && (<><a href={`/${area.slug}`} className="hover:text-blue-600">{area.name}</a><span className="mx-2">&rsaquo;</span></>)}
          {municipality && (<><a href={`/${municipality.slug}`} className="hover:text-blue-600">{municipality.name}</a><span className="mx-2">&rsaquo;</span></>)}
