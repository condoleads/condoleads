-- Snapshot BEFORE UNIT 27 (tenant_assistant role + house_account eligible).
-- Restore via: psql -f <this file>

-- Restore agents_role_check:
ALTER TABLE public.agents DROP CONSTRAINT agents_role_check;
ALTER TABLE public.agents ADD CONSTRAINT agents_role_check CHECK ((role = ANY (ARRAY['agent'::text, 'manager'::text, 'area_manager'::text, 'tenant_admin'::text, 'admin'::text, 'assistant'::text])));

-- Restore validate_house_account function body:
