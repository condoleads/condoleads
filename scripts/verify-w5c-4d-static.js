#!/usr/bin/env node
/**
 * verify-w5c-4d-static.js
 *
 * Static post-patch verification for W-LEADS-WORKBENCH W5c-4d:
 * migrate app/api/walliam/charlie/vip-approve/route.ts to the shared
 * approveVipRequest helper.
 *
 * Read-only. Exits 0 if all PASS, 1 if any FAIL, 2 on fatal (missing file).
 *
 * Pattern matches scripts/verify-w5c-4c-static.js. Charlie-specific deltas
 * vs W5c-4c:
 *   - Brand load via getTenantContext (NOT direct tenants SELECT)
 *   - HTML response icons as HTML entities (NOT Unicode escapes)
 *   - Title format `${brand ? brand + ' - ' : ''}${title}` (NOT em-dash)
 *   - Approve message "Plan access granted to" (NOT "Estimator access")
 *   - Deny message "VIP request from" (NOT "Estimator VIP request from")
 *   - estimatorBccFailurePolicy: 'fail-open' (NOT 'fail-closed')
 */
...
// ============================================================
// W5c-4d MIGRATION MARKERS (header block)
// ============================================================
check(
  'header: W5c-4d marker comment present',
  text.indexOf('W-LEADS-WORKBENCH W5c-4d (2026-05-16)') !== -1,
  'expected migration header marker',
)
check(
  'header: F-W5C-4D-ASCII-COMMENTS finding logged',
  text.indexOf('F-W5C-4D-ASCII-COMMENTS') !== -1,
  'expected ascii-comments finding in header',
)
check(
  'header: F-W5C-4D-EMPTY-TENANT-GUARD-ADDED finding logged',
  text.indexOf('F-W5C-4D-EMPTY-TENANT-GUARD-ADDED') !== -1,
  'expected empty-tenant-guard finding in header',
)
check(
  'header: legacy "audit OMITTED" rationale documented',
  text.indexOf('audit OMITTED') !== -1,
  'expected audit-omission rationale in header or call site',
)
...
// ============================================================
check(
  'imports: approveVipRequest imported from @/lib/admin-homes/approve-vip-request',
...
check(
  'imports: getTenantContext + buildBaseUrl preserved from @/lib/utils/tenant-brand',
  /import\s*\{[\s\S]*?getTenantContext[\s\S]*?buildBaseUrl[\s\S]*?\}\s*from\s*'@\/lib\/utils\/tenant-brand'/.test(
    text,
  ),
  'expected both getTenantContext + buildBaseUrl imports preserved (Charlie uses helper, not direct SELECT)',
)
check(
...
check(
  "call: estimatorBccFailurePolicy: 'fail-open' (Charlie-specific; irrelevant for plan/chat per helper L155)",
  text.indexOf("estimatorBccFailurePolicy: 'fail-open'") !== -1,
  "expected 'fail-open' literal in helper call (NOT 'fail-closed' which is estimator-specific)",
)
check(
  "call: estimatorBccFailurePolicy is NOT 'fail-closed' (that's the estimator route W5c-4c value)",
  text.indexOf("estimatorBccFailurePolicy: 'fail-closed'") === -1,
  "Charlie must NOT pass 'fail-closed' (per helper docstring, value is irrelevant but we pass 'fail-open' for clarity)",
)
check(
...
// audit param must be OMITTED (Charlie legacy did not write lead_admin_actions)
...
const auditPassed = approveCallMatch
  ...
  : false
check(
  'call: audit param OMITTED (legacy Charlie does not write lead_admin_actions)',
  !auditPassed,
  'audit must not be passed to helper',
)
...
check(
  ...
  text.indexOf('function createServiceClient()') !== -1 &&
    text.replace(/\/\/[^\n]*/g, '').indexOf("from '@/lib/admin-homes/service-client'") === -1,
...
check(
  ...
  'expected single-gate by approval_token',
)
check(
  'route: brand load via getTenantContext (NOT direct tenants SELECT)',
  text.indexOf('getTenantContext(supabase,') !== -1 &&
    text.replace(/\/\/[^\n]*/g, '').indexOf(".from('tenants')") === -1,
  'Charlie preserves legacy getTenantContext pattern; direct tenants SELECT must NOT appear in active code',
)
check(
...
check(
  'route: empty-tenant guard (F-W5C-4D-EMPTY-TENANT-GUARD-ADDED)',
...
check(
  "html: configs.approved -> #10b981, '&#10003;', 'Approved' (HTML entity check mark)",
  /approved:\s*\{\s*bg:\s*'#10b981',\s*icon:\s*'&#10003;',\s*title:\s*'Approved'\s*\}/.test(
    text,
  ),
  'expected approved config tuple (green + HTML entity check)',
)
check(
  "html: configs.denied -> #ef4444, '&#10007;', 'Denied' (HTML entity ballot X)",
  /denied:\s*\{\s*bg:\s*'#ef4444',\s*icon:\s*'&#10007;',\s*title:\s*'Denied'\s*\}/.test(
    text,
  ),
  'expected denied config tuple (red + HTML entity X)',
)
check(
  "html: configs.error -> #ef4444, '&#9888;', 'Error' (HTML entity warning)",
  /error:\s*\{\s*bg:\s*'#ef4444',\s*icon:\s*'&#9888;',\s*title:\s*'Error'\s*\}/.test(
    text,
  ),
  'expected error config tuple (red + HTML entity warning)',
)
check(
  "html: configs.expired -> #f59e0b, '&#8987;', 'Expired' (HTML entity hourglass)",
  /expired:\s*\{\s*bg:\s*'#f59e0b',\s*icon:\s*'&#8987;',\s*title:\s*'Expired'\s*\}/.test(
    text,
  ),
  'expected expired config tuple (amber + HTML entity hourglass)',
)
check(
  "html: configs.already_processed -> #64748b, '&#8505;', 'Already Processed' (HTML entity info)",
  /already_processed:\s*\{\s*bg:\s*'#64748b',\s*icon:\s*'&#8505;',\s*title:\s*'Already Processed'\s*\}/.test(
    text,
  ),
  'expected already_processed config tuple (slate + HTML entity info)',
)
check(
  "html: title format uses hyphen separator with conditional brand prefix",
  text.indexOf("${brandName ? brandName + ' - ' : ''}${cfg.title}") !== -1,
  "expected Charlie title format (NOT estimator's em-dash separator)",
)
check(
  "html: title does NOT use estimator's em-dash separator",
  text.indexOf('Estimator \\u2014') === -1 &&
    text.indexOf(' Estimator ') === -1,
  "Charlie route must not regress to estimator title format",
)
check(
...
// ============================================================
// PRESERVED VERBATIM: USER-FACING MESSAGES (Charlie-specific)
// ============================================================
check(
  "msg: 'Plan access granted to' (approve path; Charlie-specific wording)",
  text.indexOf('Plan access granted to') !== -1,
  "expected Charlie's approve message (NOT 'Estimator access granted')",
)
check(
  "msg: approve does NOT use estimator wording",
  text.indexOf('Estimator access granted') === -1 &&
    text.indexOf('additional estimate') === -1,
  'Charlie must not regress to estimator approve message',
)
check(
  "msg: 'additional plan' wording for grant count",
  text.indexOf('additional plan') !== -1,
  "expected Charlie's 'additional plan' message",
)
check(
  "msg: 'VIP request from' + 'has been denied' (deny path; Charlie-specific)",
  text.indexOf('VIP request from') !== -1 &&
    text.indexOf('has been denied') !== -1,
  "expected Charlie's deny message",
)
check(
  "msg: deny does NOT use estimator wording",
  text.indexOf('Estimator VIP request from') === -1,
  'Charlie must not regress to estimator deny message',
)
check(
  "msg: defensive error wording 'Approval recorded; please contact support.'",
  text.indexOf(
    'System notification failed. Approval recorded; please contact support.',
  ) !== -1,
  'expected defensive error wording (mirrors W5c-4c for UX consistency)',
)
check(
...
check(
  'isolation: no getLeadEmailRecipients import in route (BCC handled by helper)',
  text.replace(/\/\/[^\n]*/g, '').indexOf('getLeadEmailRecipients') === -1,
  'recipient helper must not appear in active code (comments OK)',
)
check(
  'isolation: no inline ADMIN_EMAIL literal',
  !/ADMIN_EMAIL\s*=\s*['"]/.test(text),
  'no inline admin email allowed',
)
check(
  'isolation: no direct sendTenantEmail / resend.emails.send in route (helper owns send)',
  text.replace(/\/\/[^\n]*/g, '').indexOf('sendTenantEmail(') === -1 &&
    text.indexOf('resend.emails.send') === -1,
  'email send must be inside approveVipRequest, not the route (comment-only references OK)',
)
check(
  'isolation: no inline buildUserApprovalEmailHtml (legacy email builder removed)',
  text.indexOf('function buildUserApprovalEmailHtml') === -1,
  'legacy inline email HTML builder must be removed in migrated version',
)

// Multitenant: no 'walliam' literal in code body (only filename)
...
// ============================================================
// ASCII PURITY (F-W5C-4D-ASCII-COMMENTS)
// ============================================================
const nonAscii = []
...
console.log('')
console.log('W5c-4d static verification:')
