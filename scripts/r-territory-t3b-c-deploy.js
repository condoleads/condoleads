// scripts/r-territory-t3b-c-deploy.js
// W-TERRITORY/T3b-C — apa triggers wiring distribution + re-roll on every routing change.
//
// Single migration creating 3 trigger functions + 3 triggers on agent_property_access:
//   - handle_apa_insert  (AFTER INSERT)  → distribute_geo_to_children + reroll
//   - handle_apa_update  (AFTER UPDATE)  → reroll only on routing-affecting changes
//   - handle_apa_delete  (AFTER DELETE)  → reroll
//
// Recursion guard: pg_trigger_depth() > 1 → skip. Built-in PG mechanism, no session
// variables. Prevents infinite loop when distribute_geo_to_children INSERTs into apa.
//
// Idempotent: refuses to overwrite migration file. CREATE OR REPLACE for functions,
// DROP TRIGGER IF EXISTS + CREATE TRIGGER for triggers. BEGIN/COMMIT atomic.
...
console.log('[T3B-C-DEPLOY] W-TERRITORY/T3b-C — 1 migration file')
console.log('[T3B-C-DEPLOY] STAMP=' + STAMP)
console.log('')

const FILE1_REL = "supabase/migrations/20260507_t3b_c_01_apa_distribution_triggers.sql"
const FILE1_LINES = [
...
const FILES = [
  { rel: FILE1_REL, lines: FILE1_LINES, label: 'T3b-C: apa distribution triggers (3 functions + 3 triggers)' },
]

let abort = false
for (const f of FILES) {
  if (fs.existsSync(path.join(PROJECT_ROOT, f.rel))) {
    console.error('[T3B-C-DEPLOY] REFUSE: ' + f.rel + ' already exists')
    abort = true
  }
}
if (abort) {
  console.error('[T3B-C-DEPLOY] Aborted — no files written.')
  process.exit(1)
}
...
for (const f of FILES) {
  ...
  console.log('[T3B-C-DEPLOY] WROTE  ' + f.rel + '  (' + fs.statSync(abs).size + ' bytes)')
  console.log('[T3B-C-DEPLOY]        ' + f.label)
}

console.log('')
console.log('[T3B-C-DEPLOY] Done. APPLY ORDER (separate Supabase pastes):')
console.log('  1. Paste ' + FILE1_REL + ' contents → run')
console.log('  2. Run T3b-C verify Block A (trigger functions present)')
console.log('  3. Run T3b-C verify Block B (triggers attached to apa)')
console.log('  4. (Optional) Run T3b-C Block C (rollback-safe INSERT smoke)')