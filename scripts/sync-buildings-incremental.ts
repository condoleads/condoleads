...
function determineIDXAccess(l: any): boolean {
  return l.StandardStatus === 'Active' && l.InternetEntireListingDisplayYN === true;
}
function determineVOWAccess(l: any): boolean {
  return l.DDFYN === true;
}
...
// =====================================================
// MEDIA / ROOMS / OPEN HOUSES SAVE
// EXACT from buildings/incremental-sync
// =====================================================

function groupMediaByBaseImage(mediaArray: any[]) {
  const groups: any = {};
  const sorted = [...mediaArray].sort((a, b) => (parseInt(a.Order) || 999) - (parseInt(b.Order) || 999));
  for (const media of sorted) {
    const baseId = media.MediaURL ? media.MediaURL.split('/').pop()?.split('.')[0] || 'unknown' : 'unknown';
...
async function saveMediaForNewListings(newListings: { savedListing: any; originalListing: any }[]): Promise<number> {
  let count = 0;
  const records: any[] = [];
  for (const { savedListing, originalListing } of newListings) {
    if (!originalListing?.Media || !Array.isArray(originalListing.Media)) continue;
    const grouped = groupMediaByBaseImage(originalListing.Media);
    for (const [baseId, variants] of Object.entries(grouped)) {
      const thumb = (variants as any[]).find((v: any) => v.MediaURL && (v.MediaURL.includes('rs:fit:240:240') || v.ImageSizeDescription === 'Thumbnail'));
      const large = (variants as any[]).find((v: any) => v.MediaURL && (v.MediaURL.includes('rs:fit:1920:1920') || v.ImageSizeDescription === 'Large'));
      if (thumb) records.push(createMediaRecord(savedListing.id, thumb, 'thumbnail', baseId.substring(0, 100)));
      if (large) records.push(createMediaRecord(savedListing.id, large, 'large', baseId.substring(0, 100)));
    }
  }
  if (records.length > 0) {
    for (let i = 0; i < records.length; i += 100) {
      const batch = records.slice(i, i + 100);
      const { error: err } = await supabase.from('media').insert(batch);
      if (err) console.error(`[MEDIA] Insert error: ${err.message}`);
...
async function saveRoomsForNewListings(newListings: { savedListing: any; originalListing: any }[]): Promise<number> {
  let count = 0;
  const records: any[] = [];
  for (const { savedListing, originalListing } of newListings) {
    if (!originalListing?.PropertyRooms || !Array.isArray(originalListing.PropertyRooms)) continue;
    for (const room of originalListing.PropertyRooms) {
      records.push({
        listing_id: savedListing.id,
        room_key: room.RoomKey || null, room_type: room.RoomType || null,
        room_level: room.RoomLevel || null, room_status: room.RoomStatus || null,
        room_dimensions: room.RoomDimensions || null,
        room_length: parseDecimal(room.RoomLength), room_width: parseDecimal(room.RoomWidth),
        room_height: parseDecimal(room.RoomHeight), room_area: parseDecimal(room.RoomArea),
        room_length_width_units: room.RoomLengthWidthUnits || null,
        room_length_width_source: room.RoomLengthWidthSource || null,
        room_area_units: room.RoomAreaUnits || null, room_area_source: room.RoomAreaSource || null,
        room_description: room.RoomDescription || null,
        room_features: parseJsonArray(room.RoomFeatures),
        room_feature1: room.RoomFeature1 || null, room_feature2: room.RoomFeature2 || null,
  ...
  if (records.length > 0) {
    for (let i = 0; i < records.length; i += 100) {
      const batch = records.slice(i, i + 100);
      const { error: err } = await supabase.from('property_rooms').insert(batch);
      if (err) console.error(`[ROOMS] Insert error: ${err.message}`);
      else count += batch.length;
    }
  }
  return count;
...
async function saveOpenHousesForNewListings(newListings: { savedListing: any; originalListing: any }[]): Promise<number> {
  let count = 0;
  const records: any[] = [];
  for (const { savedListing, originalListing } of newListings) {
    if (!originalListing?.OpenHouses || !Array.isArray(originalListing.OpenHouses)) continue;
    for (const oh of originalListing.OpenHouses) {
      records.push({
        listing_id: savedListing.id,
        open_house_key: oh.OpenHouseKey || null, open_house_id: oh.OpenHouseID || null,
        open_house_date: parseDate(oh.OpenHouseDate),
        open_house_start_time: parseTimestamp(oh.OpenHouseStartTime),
        open_house_end_time: parseTimestamp(oh.OpenHouseEndTime),
        open_house_type: oh.OpenHouseType || null, open_house_status: oh.OpenHouseStatus || null,
        original_entry_timestamp: parseTimestamp(oh.OriginalEntryTimestamp),
        modification_timestamp: parseTimestamp(oh.ModificationTimestamp),
        created_at: new Date().toISOString()
      });
    }
  }
  if (records.length > 0) {
    for (let i = 0; i < records.length; i += 100) {
      const batch = records.slice(i, i + 100);
      const { error: err } = await supabase.from('open_houses').insert(batch);
      if (err) console.error(`[OH] Insert error: ${err.message}`);
      else count += batch.length;
    }
  }
  return count;
...
// =====================================================
// PROPTX FETCH FOR SINGLE BUILDING
// EXACT from buildings/incremental-sync fetchPropTxListings
// =====================================================

async function fetchPropTxListingsForBuilding(building: any): Promise<any[]> {
  ...
  if (!streetNumber) throw new Error('Street number is required for search');
  ...
  const activeResults = await fetchWithRetry(activeUrl, { headers }, 3, `Building active: ${building.building_name}`)
  ...
  const completedResults = await fetchWithRetry(completedUrl, { headers }, 3, `Building completed: ${building.building_name}`)
    .then(r => r.json()).then(d => d.value || []).catch(() => []);
  allListings.push(...completedResults);

  // Deduplicate
  const seen = new Set<string>();
  let unique = allListings.filter(l => {
    const key = l.ListingKey || `${l.StreetNumber}-${l.UnitNumber}-${l.MlsStatus}`;
    if (seen.has(key)) return false;
    seen.add(key); return true;
  });

  // Filter 1: Street Number exact
  unique = unique.filter(l => l.StreetNumber === streetNumber);
  // Filter 2: Street Name first word
  if (streetName?.trim()) {
    const sw = streetName.toLowerCase().trim().split(' ')[0];
    unique = unique.filter(l => (l.UnparsedAddress || '').toLowerCase().includes(sw) || (l.StreetName || '').toLowerCase().includes(sw));
  }
  // Filter 3: City first word
  if (city?.trim()) {
...
async function syncOneBuilding(building: any, triggeredBy: string): Promise<{
  ...
  const result = { added: 0, updated: 0, removed: 0, unchanged: 0, mediaAdded: 0, roomsAdded: 0, openHousesAdded: 0 };

  const proptxListings = await fetchPropTxListingsForBuilding(building);
  ...
  const { data: dbListings } = await supabase.from('mls_listings').select('*').eq('building_id', building.id);
  ...
  if (proptxActive.length === 0 && dbActive.length > 0) {
    ...
    for (const [key, ptx] of proptxMap) {
      ...
      if (db) {
        const priceChanged = (parseInt(ptx.ListPrice) || 0) !== (db.list_price || 0);
        const statusChanged = ptx.StandardStatus !== db.standard_status;
        if (priceChanged || statusChanged) {
          await supabase.from('mls_listings').update({
            list_price: parseInt(ptx.ListPrice) || 0,
            standard_status: ptx.StandardStatus,
            mls_status: ptx.MlsStatus,
            updated_at: new Date().toISOString()
          }).eq('id', db.id);
          result.updated++;
        } else {
    ...
    // INSERT new active
    const newActive: { savedListing: any; originalListing: any }[] = [];
    for (const [key, ptx] of proptxMap) {
      if (!dbMap.has(key)) {
    ...
    if (newActive.length > 0) {
      ...
      result.mediaAdded += await saveMediaForNewListings(newActive);
      result.roomsAdded += await saveRoomsForNewListings(newActive);
      result.openHousesAdded += await saveOpenHousesForNewListings(newActive);
    }
  }
  ...
  if (newInactive.length > 0) {
    await fetchEnhancedDataForBuildings(newInactive.map(n => n.originalListing));
    result.mediaAdded += await saveMediaForNewListings(newInactive);
    result.roomsAdded += await saveRoomsForNewListings(newInactive);
    result.openHousesAdded += await saveOpenHousesForNewListings(newInactive);
  }

  // Backfill geo IDs + update building timestamp
  await backfillListingGeoIds(building.id);
  await supabase.from('buildings').update({ last_synced_at: new Date().toISOString(), sync_status: 'completed' }).eq('id', building.id);

  // Write sync_history
  const duration = (Date.now() - startTime) / 1000;
  await writeBuildingSyncHistory({
    buildingId: building.id,
    listingsFound: proptxListings.length,
    listingsCreated: result.added,
    listingsUpdated: result.updated,
    listingsUnchanged: result.unchanged,
    mediaCount: result.mediaAdded,
    roomCount: result.roomsAdded,
    openHouseCount: result.openHousesAdded,
    duration,
    triggeredBy,
  });
...
export async function runBuildingsIncremental(triggeredBy = 'github-nightly'): Promise<{
  success: number;
  failed: number;
  skipped: number;
}> {
  ...
  try {
    ...
    log(TAG, `Starting incremental sync for ${buildings.length} buildings`);

    for (let idx = 0; idx < buildings.length; idx++) {
      const bldg = buildings[idx];
      try {
      ...
      // Rate limit delay between buildings
      if (idx < buildings.length - 1) {
        await delay(DELAY_BETWEEN_BUILDINGS_MS);
      }
    }
  } catch (err: any) {
    error(TAG, `Fatal: ${err.message}`);
    throw err;
  }

  log(TAG, `Complete: ${results.success} success, ${results.failed} failed`);
  return results;
}

// =====================================================
// STANDALONE EXECUTION
// Usage: npx tsx scripts/sync-buildings-incremental.ts
// =====================================================

if (require.main === module) {
  runBuildingsIncremental('github-manual')
    .then(results => {
      console.log('\n=== BUILDINGS INCREMENTAL RESULTS ===');
