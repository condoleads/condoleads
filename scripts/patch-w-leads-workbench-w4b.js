// scripts/patch-w-leads-workbench-w4b.js
// W-LEADS-WORKBENCH W4b: Plan tab renderer.
// Creates PlanRenderer.tsx, patches LeadWorkbenchClient.tsx + page.tsx.
// Tracker is patched in a separate follow-up script (W4b-tracker).
...
const PLAN_RENDERER_REL = 'components/admin-homes/lead-workbench/PlanRenderer.tsx'
const WORKBENCH_CLIENT_REL = 'app/admin-homes/leads/[id]/LeadWorkbenchClient.tsx'
...
// =============================================================================
// PlanRenderer.tsx content
// =============================================================================

const PLAN_RENDERER_CONTENT = `'use client'
...
console.log('=== W4b Patch ===')
console.log('Timestamp: ' + ts)
console.log()

backup(WORKBENCH_CLIENT_REL)
backup(PAGE_REL)

// 1. Create PlanRenderer.tsx
const planRendererAbs = path.join(ROOT, PLAN_RENDERER_REL)
fs.writeFileSync(planRendererAbs, PLAN_RENDERER_CONTENT, 'utf8')
console.log('  CREATE ' + PLAN_RENDERER_REL + ' (' + PLAN_RENDERER_CONTENT.length + ' bytes)')

// 2. Patch LeadWorkbenchClient.tsx
{
  const abs = path.join(ROOT, WORKBENCH_CLIENT_REL)
  ...
  txt = exactReplace(txt, oldImport, newImport, 'LeadWorkbenchClient import block')

  const oldTernary = "        {tab === 'overview' ? (\n          <OverviewTab anchorLead={anchorLead} leadFamily={leadFamily} />\n        ) : (\n          <PlaceholderTab name={activeTabMeta.label} phase={activeTabMeta.phase} />\n        )}"
  const newTernary = "        {tab === 'overview' ? (\n          <OverviewTab anchorLead={anchorLead} leadFamily={leadFamily} />\n        ) : tab === 'plan' ? (\n          <PlanTab anchorLead={anchorLead} leadFamily={leadFamily} />\n        ) : (\n          <PlaceholderTab name={activeTabMeta.label} phase={activeTabMeta.phase} />\n        )}"
  txt = exactReplace(txt, oldTernary, newTernary, 'LeadWorkbenchClient tab ternary')

  fs.writeFileSync(abs, txt, 'utf8')
  console.log('  PATCH  ' + WORKBENCH_CLIENT_REL + ' (import + ternary extension)')
}
...
// 3. Patch page.tsx
{
  ...
  const oldAnchor = "const ANCHOR_SELECT = '*, agents!leads_agent_id_fkey(id, full_name, email), manager:agents!leads_manager_id_fkey(id, full_name, email), area_manager:agents!leads_area_manager_id_fkey(id, full_name, email), tenant_admin:agents!leads_tenant_admin_id_fkey(id, full_name, email)'"
  const newAnchor = "const ANCHOR_SELECT = '*, agents!leads_agent_id_fkey(id, full_name, email, cell_phone, profile_photo_url, brokerage_name, title), manager:agents!leads_manager_id_fkey(id, full_name, email), area_manager:agents!leads_area_manager_id_fkey(id, full_name, email), tenant_admin:agents!leads_tenant_admin_id_fkey(id, full_name, email)'"
  txt = exactReplace(txt, oldAnchor, newAnchor, 'page.tsx ANCHOR_SELECT')

  const oldFamily = "const FAMILY_SELECT = '*, agents!leads_agent_id_fkey(id, full_name, email)'"
  const newFamily = "const FAMILY_SELECT = '*, agents!leads_agent_id_fkey(id, full_name, email, cell_phone, profile_photo_url, brokerage_name, title)'"
  txt = exactReplace(txt, oldFamily, newFamily, 'page.tsx FAMILY_SELECT')

  fs.writeFileSync(abs, txt, 'utf8')
  console.log('  PATCH  ' + PAGE_REL + ' (ANCHOR_SELECT + FAMILY_SELECT agent fields)')
}

console.log()
console.log('=== Patch complete. Run tsc --noEmit next ===')