// scripts/r-territory-t4c-1-route-deploy.js
//
// T4c-1 Phase A deploy:
//   - package.json   : add pg to dependencies (it's currently devDep only;
//                      Vercel/Next.js production runtime won't have it)
//   - app/api/admin-homes/territory/bulk-assign/route.ts (NEW)
//
// Smoke is a separate deploy in Phase B.
//
// Run: node scripts/r-territory-t4c-1-route-deploy.js
// Then: npm install pg --save  (only if package.json was edited)
// Then: npx tsc --noEmit
...
// ============================================================================
// Step 1: package.json -- ensure pg is in dependencies
// ============================================================================
...
if (pkg.dependencies.pg) {
  console.log('PKG SKIP: pg already in dependencies (' + pkg.dependencies.pg + ')');
} else {
  const pgVersion = (pkg.devDependencies && pkg.devDependencies.pg) || '^8.20.0';
  fs.writeFileSync(PKG_PATH + '.backup_' + STAMP, pkgRaw, 'utf8');
  console.log('  backup: ' + PKG_PATH + '.backup_' + STAMP);
  const newDeps = Object.assign({}, pkg.dependencies);
  newDeps.pg = pgVersion;
  pkg.dependencies = Object.fromEntries(
    Object.entries(newDeps).sort(function (a, b) { return a[0].localeCompare(b[0]); })
  );
  fs.writeFileSync(PKG_PATH, JSON.stringify(pkg, null, 2) + '\n', 'utf8');
  console.log('PKG OK: pg ' + pgVersion + ' added to dependencies');
  pkgEdited = true;
}
...
const ROUTE_PATH = 'app/api/admin-homes/territory/bulk-assign/route.ts';
...
const target = path.resolve(ROUTE_PATH);
const dir = path.dirname(target);

if (fs.existsSync(target)) {
  console.error('FAIL: ' + ROUTE_PATH + ' already exists. T4c-1 Phase A is NEW only.');
  console.error('       To re-deploy, delete or move the existing file first.');
  process.exit(1);
}

try {
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(target, ROUTE_CONTENT, { encoding: 'utf8', flag: 'wx' });
  const size = fs.statSync(target).size;
  console.log('NEW OK: ' + ROUTE_PATH + ' (' + size + ' bytes)');
} catch (e) {
  console.error('FAIL: ' + ROUTE_PATH + ' -- ' + e.message);
  process.exit(1);
}

console.log('');
console.log('Summary:');
console.log('  package.json edited: ' + (pkgEdited ? 'YES (pg added to dependencies)' : 'no (already present)'));
console.log('  route.ts:            written (NEW)');
console.log('');
console.log('Next:');
if (pkgEdited) {
  console.log('  npm install pg --save     # update package-lock.json');
}
console.log('  npx tsc --noEmit          # verify route compiles');
console.log('  # then save the smoke artifact in the next message and run Phase B deploy');