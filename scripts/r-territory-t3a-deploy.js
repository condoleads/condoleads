// scripts/r-territory-t3a-deploy.js
// W-TERRITORY/T3a — resolver baseline + v2 refactor (2 migrations).
//
//   01: capture baseline (CREATE OR REPLACE both existing functions verbatim)
//   02: drop + recreate with v2 logic — is_primary aware, neighbourhood param,
//       no managed-child substitution, modern tenant_users path; new helpers
//       resolve_geo_primary + pick_routing_agent.
//
// Idempotent: refuses to overwrite existing files. Each migration is BEGIN/COMMIT atomic.
...
console.log('[T3A-DEPLOY] W-TERRITORY/T3a — 2 migration files')
console.log('[T3A-DEPLOY] STAMP=' + STAMP)
console.log('')

// ────────────────────────────────────────────────────────────────────────────
// Migration 01 — resolver baseline (capture current state)
// ────────────────────────────────────────────────────────────────────────────
const FILE1_REL = "supabase/migrations/20260507_t3_01_resolver_baseline.sql"
const FILE1_LINES = [
...
// ────────────────────────────────────────────────────────────────────────────
// Migration 02 — refactored resolvers v2 + new helpers
// ────────────────────────────────────────────────────────────────────────────
const FILE2_REL = "supabase/migrations/20260507_t3_02_resolvers_v2.sql"
const FILE2_LINES = [
...
const FILES = [
  { rel: FILE1_REL, lines: FILE1_LINES, label: 'T3a-01: resolver baseline (capture pre-T3 state)' },
  { rel: FILE2_REL, lines: FILE2_LINES, label: 'T3a-02: resolvers v2 + helpers (resolve_geo_primary, pick_routing_agent)' },
]

let abort = false
for (const f of FILES) {
  if (fs.existsSync(path.join(PROJECT_ROOT, f.rel))) {
    console.error('[T3A-DEPLOY] REFUSE: ' + f.rel + ' already exists')
    abort = true
  }
}
if (abort) {
  console.error('[T3A-DEPLOY] Aborted — no files written.')
  process.exit(1)
}
...
for (const f of FILES) {
  ...
  console.log('[T3A-DEPLOY] WROTE  ' + f.rel + '  (' + fs.statSync(abs).size + ' bytes)')
  console.log('[T3A-DEPLOY]        ' + f.label)
}

console.log('')
console.log('[T3A-DEPLOY] Done. APPLY ORDER (separate Supabase pastes):')
console.log('  1. Paste ' + FILE1_REL + ' contents → run')
console.log('  2. Run T3a-01 verify (separate block)')
console.log('  3. Paste ' + FILE2_REL + ' contents → run')
console.log('  4. Run T3a-02 verify blocks (separate, one per query)')