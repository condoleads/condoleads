// scripts/r-territory-t3b-b-deploy.js
// W-TERRITORY/T3b-B — distribution + re-roll + re-resolve PL/pgSQL functions.
//
// Single migration creating 4 functions (CREATE OR REPLACE, idempotent):
...
//      → Force-recompute cache for ALL listings in geo (used when routing changes).
//   4. reresolve_listing(listing_id, tenant_id)
//      → Single-listing recompute (used when scope shrinks).
//
// Idempotent at file level (refuses to overwrite). BEGIN/COMMIT atomic.
...
console.log('[T3B-B-DEPLOY] W-TERRITORY/T3b-B — 1 migration file')
console.log('[T3B-B-DEPLOY] STAMP=' + STAMP)
console.log('')

const FILE1_REL = "supabase/migrations/20260507_t3b_b_01_distribution_functions.sql"
const FILE1_LINES = [
...
const FILES = [
  { rel: FILE1_REL, lines: FILE1_LINES, label: 'T3b-B: distribution + re-roll + re-resolve functions (4 funcs)' },
]

let abort = false
for (const f of FILES) {
  if (fs.existsSync(path.join(PROJECT_ROOT, f.rel))) {
    console.error('[T3B-B-DEPLOY] REFUSE: ' + f.rel + ' already exists')
    abort = true
  }
}
if (abort) {
  console.error('[T3B-B-DEPLOY] Aborted — no files written.')
  process.exit(1)
}
...
for (const f of FILES) {
  ...
  console.log('[T3B-B-DEPLOY] WROTE  ' + f.rel + '  (' + fs.statSync(abs).size + ' bytes)')
  console.log('[T3B-B-DEPLOY]        ' + f.label)
}

console.log('')
console.log('[T3B-B-DEPLOY] Done. APPLY ORDER (separate Supabase pastes):')
console.log('  1. Paste ' + FILE1_REL + ' contents → run')
console.log('  2. Run T3b-B verify Block A (function existence)')
console.log('  3. (Optional) Run T3b-B Block B (preview smoke) and Block C (execute smoke)')