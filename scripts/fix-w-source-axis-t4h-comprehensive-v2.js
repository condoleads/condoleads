...
(async () => {
  ...
  try {
    ...
    try {
      ...
      info('user_profiles probe error: ' + e.message + ' — using fallback ' + FALLBACK_USER_ID);
    }
  ...
  // ----- F1: workbench page.tsx — ANCHOR_SELECT entity JOIN expansion -----
  ...
  // ----- F3: leads-list page — stray-comma anomaly (NON-FATAL) -----
  ...
  if (work.leadsListPage.includes(F3_VERIFY)) {
    ...
    else if (f3a || f3b)   info('F3 PARTIAL: applied ' + (f3a ? 'F3a ' : '') + (f3b ? 'F3b' : '') + ' — other anchor not found, cosmetic remainder');
    else {
  ...
  const tracker = [
    ...
    '| T1 — Decision lock | CLOSED | source-display reads `lead_origin_route` |',
    '| T2 — Schema migration | VACATED | columns already present |',
    '| T3 — Write-path patches | VACATED | writes already populate `lead_origin_route` |',
    '| T4-a..T4-g | CLOSED | shipped 2026-05-16/17 |',
    '| T4-h Patch A + B v2 | CLOSED 2026-05-17 | h.1–h.7 applied; h.8 verifier 68/68 PASS |',
    '| T4-h-fix (F1/F2/F3) | CLOSED ' + d.toISOString().slice(0,10) + ' | this batch |',
    '| T5 — Multi-tenant smoke | IN PROGRESS | h.3 helper 25/25; comprehensive test data shipped |',
    '| T6 — Close + master tracker | OPEN | deferred per Shah directive until W-TERRITORY T7 |',
    '',
    ...
    '- **F3** `app/admin-homes/leads/page.tsx` — stray-comma SELECT anomaly cleaned (NON-FATAL — SKIP if anchors do not match exactly; cosmetic only).',
    '',
    '## Test data inventory',
    '',
    ...
    '',
    '- F-AGENTS-PAGE-SOURCE-LIKE-TENANT-PROXY — `app/admin-homes/agents/page.tsx` `.like(\'source\', \'walliam_%\')`; breaks at tenant #2.',
    '- F-SOURCE-COLUMN-VERBOSE-TENANT-PREFIX — raw `source` stores tenant-prefixed values; display layer does not read it; cosmetic.',
    '- T6 deferral — master `W-LAUNCH-TRACKER.md` update held until W-TERRITORY T7 close.',
    '',
