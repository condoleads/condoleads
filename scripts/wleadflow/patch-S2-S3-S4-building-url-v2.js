#!/usr/bin/env node
// scripts/wleadflow/patch-S2-S3-S4-building-url-v2.js
// 4 edits removing all hardcoded '/buildings/' paths + correcting column name
// from current_page_url -> current_page_slug. LE-aware, idempotent.
...
if (buildingsCount === 0 || slugColPresent) {
  console.error('ABORT: patch appears already applied');
  process.exit(1);
}
if (buildingsCount !== 4) {
  console.error('ABORT: expected 4 occurrences of \'/buildings/\' literal, found ' + buildingsCount);
  process.exit(1);
}
...
const edit2Anchor      = "    if (chatSessionsCols.includes('current_page_url'))  minimal.current_page_url  = '/buildings/' + fx.building.slug;";
const edit2Replacement = "    if (chatSessionsCols.includes('current_page_slug')) minimal.current_page_slug = fx.building.slug;";

const edit3Anchor      = "    if ('current_page_url' in cloneS2)  cloneS2.current_page_url  = '/buildings/' + fx.building.slug;";
const edit3Replacement = "    if ('current_page_slug' in cloneS2) cloneS2.current_page_slug = fx.building.slug;";

const edit4Anchor      = "    pageUrl:       '/buildings/' + fx.building.slug,";
const edit4Replacement = "    pageUrl:       '/' + fx.building.slug,";

const edits = [
  { name: 'edit1 (first clone slug col)',  anchor: edit1Anchor, replacement: edit1Replacement },
  { name: 'edit2 (minimal-insert col)',    anchor: edit2Anchor, replacement: edit2Replacement },
  { name: 'edit3 (S2 clone slug col)',     anchor: edit3Anchor, replacement: edit3Replacement },
  { name: 'edit4 (S3 body pageUrl)',       anchor: edit4Anchor, replacement: edit4Replacement },
];
...
if (slugColCount < 3) { console.error('ABORT: expected at least 3 current_page_slug occurrences, found ' + slugColCount); process.exit(1); }
...
console.log('VERIFIED: 4 /buildings/ literals removed, ' + slugColCount + ' current_page_slug references present');