...
| F-CHARLIE-WIDGET-RENDERS-ON-SYSTEM-1 | P2 / Recon | LOGGED 2026-06-03 (surfaced during §9.2 Step 3 recon) | `CharlieWidget` is mounted globally via `components/ConditionalLayout.tsx:68` — `{isCharlieVisible && <CharlieWidget />}`. The gate is `!isAdminPage && !isDashboardPage && !isLoginPage && !is01Leads` — **no tenant / isHero / host check**. On a System 1 condoleads.ca subdomain (e.g. `yourcondorealtor.condoleads.ca`) CharlieWidget renders alongside the System 1 `ChatWidgetWrapper`. CharlieWidget's API calls (`/api/charlie/*`) require an `x-tenant-id` header set by middleware; recon needed on what value the middleware emits on a condoleads.ca subdomain request — if it defaults to or sets an arbitrary tenant id, Charlie calls could be **erroneously billed to that tenant**. If it sets nothing/empty, the visible Charlie button on System 1 just errors out on click (UX concern, not a billing one). Out of §9.2 Step 3 scope (Step 3 was strictly "stop rendering ChatWidgetWrapper on System 2"; the inverse — stop rendering CharlieWidget on System 1 — is a separate workstream). Recon-only; no build queued. |
| F-DASHBOARD-HOST-UNCLEAR | P2 / Recon | LOGGED 2026-06-03 (surfaced during Batch 1 Step 0) | The middleware sets `x-tenant-id` on `/api/*` (L113) + on the comprehensive-site rewrite (L99), but NOT on `/dashboard/*`. Implication: `lib/tenant/getTenant.ts` (which reads `x-tenant-id`) returns null in dashboard context — `getTenant()` is unreliable there. Additionally, the SYSTEM FORK at middleware L92-105 rewrites `walliam.ca/dashboard` → `/comprehensive-site/dashboard` because the host resolves to an agent with `site_type='comprehensive'`, but `app/comprehensive-site/dashboard` does NOT exist as a route → **`walliam.ca/dashboard` 404s in production**. In practice tenant agents must be reaching the dashboard via `condoleads.ca/dashboard` (the platform host), which is why the hardcoded "CondoLeads" h1 was a visible symptom for WALLiam agents. Batch 1's `getTenantBrand(agent.tenant_id)` fix correctly addresses the brand symptom by sourcing tenant identity from the agent row, not the host. But the deeper architecture question — *where should a tenant agent's dashboard live, and what does the URL bar say?* — remains unresolved. Needs recon + decision before Aily onboards real agents. Out of Batch 1 scope. |
| F-DASHBOARDLAYOUT-DEAD-CODE | Cleanup | LOGGED 2026-06-03 (surfaced during Batch 1 recon) | `components/dashboard/DashboardLayout.tsx` has **zero importers** across the codebase (verified via grep). It's a 'use client' component that exposes a sidebar layout but every `app/dashboard/*/page.tsx` renders its own inline sidebar instead. The hardcoded `'CondoLeads'` h1 at L60 inside DashboardLayout would also need brand-aware treatment if revived, but until something imports the component the dead code is the issue. Candidate for removal in a dedicated dead-code cleanup pass; not chased in Batch 1 to keep this batch scope-pure. |
