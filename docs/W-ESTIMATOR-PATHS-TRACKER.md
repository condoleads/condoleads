...
### Open items — follow-through after this commit
  ...
  [must-run-post-deploy — REGRESSION RISK, LEADS THE LIST]
    Verify the normal logged-in user can still estimate in
    production. This is THE one regression that would break the
    estimator for EVERYONE if @supabase/ssr's
    createRouteHandlerClient behaves differently behind Vercel's
    cookie handling than it does locally.

    The smoke proved both endpoints return 200 for an authed user
    on LOCAL dev with real Playwright-captured chunked
    base64-encoded cookies (sb-<projectRef>-auth-token.0 / .1).
    Vercel's edge runtime may handle cookie headers slightly
    differently — most likely the same, but the failure mode is
    catastrophic (every estimate 401s) so verify FIRST.

    HOW:
      1. Open walliam.ca in incognito; sign in as any real WALLiam
         user with remaining estimate credits.
      2. Open DevTools → Network → filter "estimator/session".
      3. Click "Get Sale Estimate" on any property page → wait for
         the buyer modal to fire its mount-effect checkAndEstimate.
      4. EXPECT: POST /api/walliam/estimator/session status=200,
         body { allowed: true, remaining: N, sessionId: <...> }.
      5. EXPECT: POST /api/walliam/estimator/increment status=200,
         body { success: true, estimatorCount: N+1 }.

    RED FLAG — if either returns 401 "Unauthorized" with a real
    signed-in user, the production cookie chain is not being
    decoded by createRouteHandlerClient. Roll back: `git revert
    94081ec c83f7b8 ea56db5` (the full working block) and ship the
    Phase 1 fix as Authorization-header-based instead of
    cookie-based. DO NOT leave production with a 401 for every
    user's estimate. The roll-back path is documented; the smoke
    proved the LOCAL dev path; the gap is the prod cookie format
    parity.

    Also verify the iteration 1a + 1b register-handoff path:
      6. In incognito, click "Get Sale Estimate" anonymously.
         The buyer modal opens the RegisterModal.
      7. Register a NEW test account.
      8. EXPECT: post-register, the buyer modal's onSuccess fires
         checkAndEstimate(confirmedUserId) — gate POST should
         carry the NEW user's id (NOT a stale closure) — status
         200, NOT 403 "Forbidden — body userId does not match
         caller".
      9. If 403, Phase 2's uidArg threading didn't take effect in
         prod (e.g. tree-shaking dropped the new param) — open a
         hotfix; the user can recover by reloading after register
         (Phase 1's identity-change clear + next loadSession does
         the right thing on reload).

  [must-run-post-deploy] Cross-account display isolation
    (the original Phase 1 reason):
    Sign in as A, navigate to /admin-homes (inert route),
    register-as-B via any RegisterModal path, assert panel does
    NOT show A's quota during the intermediate window. Without
    Phase 1, panel STAYS at A's data; with Phase 1, panel goes
    to '?' fallback then loads B.

  [must-run-post-deploy] Bleed-write prevention:
    As A, drive a real Get Estimate through the buyer modal —
    verify increment fires under A's cookie (200, A's
    chat_sessions row bumped). Then attempt to fake the body to
    send another user's userId via DevTools Network "edit and
    resend" — assert 403. Confirms the server gate fires in
    production with real cookies.

  [must-run-post-deploy] No-regression on tenant scoping:
    Try /api/walliam/estimator/session with a valid authed cookie
    but DIFFERENT tenant in x-tenant-id header — expect the tenant
    guard to still reject. The Phase 1 fix preserved that scope
    check.

  [must-run-post-deploy] VIPAIAccess panel refresh on register:
    Open walliam.ca anonymously; click "VIP AI Access" pill or
    "Join VIP Free →" full panel. Register a new account. EXPECT:
    panel updates from anonymous defaults to the new user's quotas
    WITHOUT an F5. If '?' fallback persists for >2 seconds or stays
    at anonymous, the refresh(confirmedUserId) call didn't fire in
    prod (e.g. tree-shaking).
  ...
  [SHIPPED in this working block] W-CREDIT-BLEED-PHASE2-1a + 1b.
    Threads confirmedUserId across 5 of the 14 vulnerable consumers
    (the 3 with synchronous credit actions D1+D2+D3, plus the
    VIPAIAccess nav+full mounts). 9 skip-safe sites intentionally
    untouched (Phase 1 clear covers them; documented in the 1a
    entry's "Skip-safe sites" block).

  [NEXT — separate working block] W-CREDIT-BLEED-E-TRANSITIVE.
    Thread confirmedUserId through onEstimateClick → PropertyPage
    Client `pendingUserId` prop → buyer-modal mount-effect for the
    3 ListingCard transitive sites. 5-file scope. Phase 1 protects
    the ledger; the worst-case failure on this path is a retryable
    first-mount UX blip. Real-DOM property-page smoke gate. Full
    scope spec lives in the iteration 1b entry's
    "W-CREDIT-BLEED-E-TRANSITIVE" follow-up block above.
...
═══════════════════════════════════════════════════════════════════════
W-CREDIT-BLEED-PHASE2 iteration 1a (2026-06-19) — register-handoff fix
═══════════════════════════════════════════════════════════════════════

### Goal

  Phase 1 (94081ec) closed the security/data-integrity hole — server
  now 403s any wrong-caller credit write. Phase 2 = correctness/UX
  on the safe floor: stop LEGITIMATE just-registered users from
  hitting Phase 1's 403, and eliminate the display flash on
  identity transition. Mirror the proven CharlieWidget pattern
  (thread RegisterModal.onSuccess's confirmedUserId through to
  the next credit action).

### Scope iteration 1a — 3 files

  Iteration 1a covers the highest-impact surfaces only. Iteration 1b
  (deferred to next sub-build, gated behind the EstimatorVipWrapper
  probe — see "Probe findings" below) covers EstimatorSeller D3 +
  the 3 ListingCard transitive E1-E3. Iteration 2 covers the 9
  skip-safe sites which are documented below as INTENTIONALLY
  UNTOUCHED.

### Changes

  D1 — app/estimator/components/EstimatorBuyerModal.tsx (LF):
    L19 added `import { useCreditSession } ...`.
    L70 added `const creditsCtx = useCreditSession()`.
    L177-192 widened `checkAndEstimate` signature to accept
      `uidArg?: string`; computes `const uid = uidArg ?? user?.id`;
      guards `if (!uid) { setShowRegister(true); return }`.
    L200 sessionBody now `{ userId: uid }` / `{ agentId, userId: uid,
      buildingId }` instead of `user.id`.
    L831-845 RegisterModal.onSuccess now `(confirmedUserId) => {
      setShowRegister(false);
      if (confirmedUserId) creditsCtx.refresh(undefined, confirmedUserId)
        .catch(() => {});
      checkAndEstimate(confirmedUserId);
    }`.

  D2 — app/estimator/components/HomeEstimatorBuyerModal.tsx (LF):
    Pattern-identical to D1. L12 import, L54 creditsCtx, L163-180
    checkAndEstimate widened, L180 sessionBody uses `uid`, L807-816
    onSuccess threads confirmedUserId.

  C-top — components/auth/VIPAIAccess.tsx (LF):
    L24-28 destructure widened `{ state }` → `{ state, refresh }`.
    L128-132 nav-variant RegisterModal.onSuccess: `(confirmedUserId)
      => { setShowRegister(false); if (confirmedUserId) refresh(
      undefined, confirmedUserId).catch(() => {}) }`.
    L191-195 full-variant same shape.

  Total: +73 lines / -3 lines across 3 files.

### Probe findings (gates iteration 1b — D3 + E-transitive)

  Read EstimatorVipWrapper.tsx in full this turn.

  EstimatorVipWrapper props (L9-18):
    userId: string                              ← PROP, not useAuth
    plus agentId, buildingName, buildingId,
    pageUrl, tenantId, children, etc.

  requestEstimate L177-265:
    Signature: `const requestEstimate = useCallback(async ():
      Promise<boolean> => { ... }, [agentId, userId])`
    No uid argument today.
    Gate POST at L186: `body: JSON.stringify({ agentId, userId,
      buildingId })` — sources `userId` from the CLOSURE prop.
    Increment POST at L215-222: `body: { sessionId, useSharedPool }`
      — sessionId-based, NOT userId-based (Phase 1 server matches
      session.user_id to authUser.id).

  D3 fix (iteration 1b) requires:
    (a) Widen EstimatorVipWrapper.requestEstimate signature to
        accept `(uidArg?: string) => Promise<boolean>`.
    (b) Use `(uidArg ?? userId)` in the POST body at L186.
    (c) Update EstimatorContextValue interface (L35) to reflect.
    (d) EstimatorSeller.tsx:
        - Widen handleEstimate signature to accept `(uidArg?:
          string)`.
        - Use `(uidArg ?? userId)` at L158 gate check.
        - Pass uidArg to `estimatorContext.requestEstimate(uidArg)`.
        - onSuccess at L409-412 threads confirmedUserId.

  So D3 BUILD touches 2 files: EstimatorSeller.tsx +
  EstimatorVipWrapper.tsx.

  E-transitive findings (ListingCard / HomeListingCard / GeoListingCard
  onEstimateClick callbacks):
    Signature today: `onEstimateClick?: (exactSqft: number | null)
      => void`.
    Property page mounts pass `onEstimateClick={() =>
      setShowEstimatorModal(true)}` (e.g. PropertyPageClient:113,
      225, 259, 288, 345; HomePropertyPageClient similar).
    ListingCard:442 + 403 call `onEstimateClick?.(extractExactSqft(
      listing.square_foot_source))`.
    HomeListingCard:323 + GeoListingCard:269 call `onEstimateClick
      ?.(null)`.

  E-transitive fix (iteration 1b) requires:
    (a) Widen onEstimateClick signature to `(exactSqft: number |
        null, confirmedUserId?: string) => void` in 3 ListingCard
        files (~6 line changes total).
    (b) Three ListingCard onSuccess handlers thread confirmedUserId
        through to onEstimateClick(extractExactSqft(...), confirmed
        UserId) / onEstimateClick(null, confirmedUserId).
    (c) PropertyPageClient + HomePropertyPageClient: widen
        onEstimateClick callbacks to store confirmedUserId in state,
        pass it as a new prop to EstimatorBuyerModal /
        HomeEstimatorBuyerModal (e.g. `pendingUserId={...}`).
    (d) EstimatorBuyerModal + HomeEstimatorBuyerModal mount-effect
        at L110-114 / L100-110: when modal opens via property-page
        navigation, use `(pendingUserId ?? user?.id)` for the first
        checkAndEstimate call.

  So E-transitive BUILD touches 5 files: 3 ListingCards + 2 Property
  PageClients. Plus prop-threading in the 2 buyer modals.

  Combined iteration 1b scope: 7 files (2 for D3 + 5 for E-trans).

### Smoke output (2026-06-19, local dev :3013)

  scripts/smoke-w-credit-bleed-phase2-1a.js — 5 gates, 17 sub-checks,
  all PASS:

    (1) No-regression A→A signed-in happy path:
        /session status=200, totalAllowed=15, remaining=5            PASS
    (2) B-authed body=B → 200 (Phase 2 register-handoff produces
        this shape):                                                  PASS
    (3) Phase 1 preserved: B-authed body=A → 403 (Phase 2 didn't
        loosen the server gate):                                      PASS
    (4) Code-level structural checks (11 sub-checks):                PASS
        - EstimatorBuyerModal checkAndEstimate accepts uidArg
        - EstimatorBuyerModal POSTs uid (not user.id) in sessionBody
        - EstimatorBuyerModal onSuccess threads confirmedUserId
        - EstimatorBuyerModal fires creditsCtx.refresh
        - HomeEstimatorBuyerModal checkAndEstimate accepts uidArg
        - HomeEstimatorBuyerModal POSTs uid in sessionBody
        - HomeEstimatorBuyerModal onSuccess threads confirmedUserId
        - HomeEstimatorBuyerModal fires creditsCtx.refresh
        - VIPAIAccess destructures refresh
        - VIPAIAccess nav-variant onSuccess fires refresh
        - VIPAIAccess full-variant onSuccess fires refresh
    (5) Final ledger A unchanged (no contamination):                 PASS

  TSC: `npx tsc --noEmit` exit 0.

### Skip-safe sites — INTENTIONALLY UNTOUCHED (DO NOT add useCreditSession)

  Verified untouched via `git diff` (0 lines per file):
    components/WalliamOnboardingBanner.tsx        (one-time onboarding)
    app/[slug]/components/ListingSection.tsx       (sold-listings gate)
    app/[slug]/components/NeighbourhoodListingSection.tsx (sold-gate)
    app/[slug]/components/GeoListingSection.tsx    (sold-gate)
    app/estimator/components/EstimatorSeller.tsx:92 (gating mount,
                                                     no replay; deferred
                                                     to 1b for the OTHER
                                                     mount at L409)
    components/chat/ChatLocked.tsx                 (auth-wall, not credit)
    components/auth/RequireAuth.tsx                (auth-wall, not credit)
    plus EstimatorBuyerModal:621 + HomeEstimatorBuyerModal:588 inner
      mounts that already comment "No checkAndEstimate replay"

  Reasoning: Phase 1's identity-change clear() in CreditSessionContext
  already drops state to DEFAULT_STATE on user.id transition
  (regardless of inert route), and the main useEffect's next
  evaluation on a non-inert route fires loadSession for the new
  identity. The brief '?' fallback window is acceptable on these
  surfaces because:
    - None fire a SYNCHRONOUS credit action in onSuccess (no
      checkAndEstimate / handleEstimate replay → no 403 risk).
    - WalliamOnboardingBanner is one-time (post-onboarding the
      banner is dismissed).
    - The sold-listings gates open a separate non-credit modal.
    - ChatLocked / RequireAuth are auth-walls for content gating,
      not credit gates.
    - The inner buyer-modal mounts have explicit "No replay"
      comments — they rely on the parent to remount with the
      new user.id (Phase 1's identity-change clear + main effect
      re-fire handles the panel).

  Adding useCreditSession to these surfaces is cosmetic, expands
  the import/hook surface for marginal UX gain, and increases
  surface for regression. SKIP DOCUMENTED, NOT SILENT.

### Backups in place

  Suffix `_20260619_084622`:
    app/estimator/components/EstimatorBuyerModal.tsx
    app/estimator/components/HomeEstimatorBuyerModal.tsx
    components/auth/VIPAIAccess.tsx
  Tracker:
    docs/W-ESTIMATOR-PATHS-TRACKER.md.backup_w_credit_phase2_1a_20260619_085500
...
### Iteration 1b — IMMEDIATE NEXT (builds in this same working block)

  Per the probe findings above, 1b BUILD touches 7 files:
    EstimatorSeller.tsx (D3)
    components/estimator/EstimatorVipWrapper.tsx (D3, widen
      requestEstimate signature)
    app/[slug]/components/ListingCard.tsx (E1, widen onEstimateClick)
    app/[slug]/components/HomeListingCard.tsx (E2)
    app/[slug]/components/GeoListingCard.tsx (E3)
    app/property/[id]/PropertyPageClient.tsx (route through to inner modal)
    app/property/[id]/HomePropertyPageClient.tsx (same)

  Plus prop-threading in EstimatorBuyerModal + HomeEstimatorBuyerModal
  to accept `pendingUserId` and pass it through the mount-effect
  to the first checkAndEstimate call.

### Commit gate

  W-CREDIT-BLEED-PHASE2-1a: STOPPED at commit gate. Do NOT commit
  yet — iteration 1b builds next in this same working block; Phase 1
  + 2 commit/push together as one logical unit per operator
  directive.

═══════════════════════════════════════════════════════════════════════
W-CREDIT-BLEED-PHASE2 iteration 1b (2026-06-19) — D3 seller-path fix
═══════════════════════════════════════════════════════════════════════

### Goal

  Close the D3 leg of Phase 2: thread confirmedUserId through the
  SELLER path (EstimatorSeller → estimatorContext.requestEstimate
  → gate POST). Same shape as D1/D2 from iteration 1a — uidArg ?? closure.

### Scope iteration 1b — 2 files

  components/estimator/EstimatorVipWrapper.tsx (LF) — D3a
  app/estimator/components/EstimatorSeller.tsx (CRLF) — D3b

  E-transitive (5 files: ListingCard/HomeListingCard/GeoListingCard +
  PropertyPageClient + HomePropertyPageClient) SPLIT OUT to a separate
  named follow-up — see "W-CREDIT-BLEED-E-TRANSITIVE" below.

### Changes

  D3a — components/estimator/EstimatorVipWrapper.tsx (LF):
    L35-43  EstimatorContextValue interface widened —
      `requestEstimate: () => Promise<boolean>` →
      `requestEstimate: (uidArg?: string) => Promise<boolean>`.
      8-line W-CREDIT-BLEED-PHASE2-1b comment explains the race.
    L185-194 requestEstimate signature widened —
      `async (): Promise<boolean>` → `async (uidArg?: string):
      Promise<boolean>`.
      Computes `const effectiveUserId = uidArg ?? userId` at the
      top of the try block (where userId is the prop closure).
    L201 gate POST body now uses `userId: effectiveUserId`
      instead of bare `userId` — same key in the JSON, fresh
      value when uidArg is provided.
    useCallback deps `[agentId, userId]` UNCHANGED (uidArg is a
    parameter, not a closure dep).
    Increment POST at L215-222 UNTOUCHED — sessionId-based, Phase 1
    already protects it via session.user_id === authUser.id.

  D3b — app/estimator/components/EstimatorSeller.tsx (CRLF):
    L156-179 handleEstimate widened —
      `async () => {` → `async (uidArg?: string) => {`.
      Computes `const effectiveUserId = uidArg ?? userId` (userId is
      the prop sourced from useAuth in the wrapping EstimatorSellerOuter).
      `if (!effectiveUserId)` instead of `if (!userId)`.
    L177 estimatorContext.requestEstimate(uidArg) — threaded through.
    L374 button onClick wrapped — `onClick={handleEstimate}` →
      `onClick={() => handleEstimate()}` to prevent React from
      passing the MouseEvent into the new uidArg parameter slot.
      Caught by TSC: `Type 'MouseEvent<HTMLButtonElement,
      MouseEvent>' is not assignable to type 'string'.` Wrapped
      callback fixes it without changing the seller's normal flow.
    L418-425 onSuccess at RegisterModal mount —
      `() => { setShowRegister(false); handleEstimate() }` →
      `(confirmedUserId) => { setShowRegister(false);
      handleEstimate(confirmedUserId) }`.

### Smoke output (2026-06-19, local dev :3014)

  scripts/smoke-w-credit-bleed-phase2-1b.js — 6 gates, 20 sub-checks,
  all PASS:

    (1) No-regression: signed-in A path (no uidArg) → 200          PASS
    (2) Phase 2 happy: B-authed-B-bodied → 200 (the seller register-
        handoff equivalent — confirmedUserId routes through to the
        gate POST avoiding Phase 1's 403)                            PASS
    (3) Phase 1 preserved: B-authed body=A → 403 (1b didn't loosen) PASS
    (4) Code-level structural proof (9/9 sub-checks)                PASS
        - EstimatorVipWrapper interface widened to (uidArg?: string)
        - requestEstimate signature accepts uidArg
        - effectiveUserId = uidArg ?? userId computed
        - Gate POST uses userId: effectiveUserId
        - EstimatorSeller handleEstimate accepts uidArg
        - effectiveUserId fallback computed
        - estimatorContext.requestEstimate(uidArg) threaded
        - onSuccess threads confirmedUserId → handleEstimate
        - Button onClick wrapped (no MouseEvent leak)
    (5) 1a no-regression: 1a markers intact across the 3 buyer-
        modal/VIPAIAccess files (3/3 sub-checks)                    PASS
    (6) A's ledger UNCHANGED across smoke (no contamination)        PASS

  TSC: `npx tsc --noEmit` exit 0.

### TSC-caught issue fixed mid-build

  After applying the patch, TSC caught one type mismatch:
    EstimatorSeller.tsx(374,15): TS2322:
      Type '(uidArg?: string) => Promise<void>' is not assignable
      to type 'MouseEventHandler<HTMLButtonElement>'.
      Type 'MouseEvent<HTMLButtonElement, MouseEvent>' is not
      assignable to type 'string'.
  The submit button's `onClick={handleEstimate}` was passing the
  MouseEvent into the new uidArg parameter slot. Fixed by wrapping
  with `onClick={() => handleEstimate()}` — the seller's normal
  click flow is unchanged; the wrapper just discards the event.

### Scope verification — exactly 2 files (1b)

  Modified (git diff per file):
    components/estimator/EstimatorVipWrapper.tsx        (D3a)
    app/estimator/components/EstimatorSeller.tsx        (D3b)

  E-transitive UNTOUCHED — 0 diff lines per file:
    app/[slug]/components/ListingCard.tsx
    app/[slug]/components/HomeListingCard.tsx
    app/[slug]/components/GeoListingCard.tsx
    app/property/[id]/PropertyPageClient.tsx
    app/property/[id]/HomePropertyPageClient.tsx

  Skip-safe UNTOUCHED — 0 diff lines per file:
    components/WalliamOnboardingBanner.tsx
    app/[slug]/components/ListingSection.tsx
    app/[slug]/components/NeighbourhoodListingSection.tsx
    app/[slug]/components/GeoListingSection.tsx
    components/chat/ChatLocked.tsx
    components/auth/RequireAuth.tsx

### Backups in place

  Suffix `_20260619_093017`:
    components/estimator/EstimatorVipWrapper.tsx
    app/estimator/components/EstimatorSeller.tsx
  Tracker:
    docs/W-ESTIMATOR-PATHS-TRACKER.md.backup_w_credit_phase2_1b_20260619_094500
...
### Named follow-up — W-CREDIT-BLEED-E-TRANSITIVE (NEXT WORKING BLOCK)

  Thread confirmedUserId through the property-page render path:
    onEstimateClick callback signature → PropertyPageClient state
    → EstimatorBuyerModal new prop `pendingUserId` → mount-effect
    uses `(pendingUserId ?? user?.id)` for the first checkAndEstimate.

  Scope: 5 files
    app/[slug]/components/ListingCard.tsx           (widen onEstimateClick)
    app/[slug]/components/HomeListingCard.tsx       (same)
    app/[slug]/components/GeoListingCard.tsx        (same)
    app/property/[id]/PropertyPageClient.tsx        (route through to inner modal)
    app/property/[id]/HomePropertyPageClient.tsx    (same)
  Plus: prop-threading in EstimatorBuyerModal + HomeEstimatorBuyerModal
  to accept and use `pendingUserId` in the mount-effect.

  Why NOT in this push:
    - Phase 1's server gate ALREADY blocks the ledger contamination
      (the only safety failure mode). The remaining E-transitive
      failure is a retryable first-mount UX blip (the modal's first
      checkAndEstimate after register-from-property-card hits 403,
      then the second fire after AuthContext propagates succeeds —
      visible as a "Failed to check access" toast that disappears
      on retry).
    - PropertyPageClient + HomePropertyPageClient are the highest-
      regression surface in the property-page render path. Adding
      state + new prop threading + buyer-modal mount-effect changes
      touches code paths that render on EVERY property visit. Real-
      DOM verification on a deployed walliam.ca property page is the
      right gate.
    - Splitting it out means this push can ship Phase 1 + 2 (1a + 1b)
      with high confidence, and the E-transitive cleanup ships next
      working block with its own real-DOM property-page smoke.

  NAMED, scoped, next-block. NOT indefinitely deferred. Tracker
  entry placeholder so the work-item doesn't drift.

### Commit gate

  W-CREDIT-BLEED-PHASE2-1b: STOPPED at commit gate. Iteration 1b
  builds in this same working block as 1a; Phase 1 + 1a + 1b
  commit/push together as one logical unit per operator directive.
