...
- **F-FLOOR-POOL-PERMISSION-DENIED (OPEN, bounded)** — Surfaced in EDIT C.5 sub-smoke 2 and re-confirmed during E2E S4: POST to `/api/walliam/assign-user-agent` with a `listing_id` whose cache is NULL returns HTTP 500 "permission denied for table `tenant_floor_pool`". RLS denies the service-role-impersonating route the SELECT it needs inside `resolve_agent_for_context` for the floor branch. **Bounded** — only affects listings where `assigned_agent_id IS NULL` (cache-miss into the RPC's floor path); cache-hit listings skip this code path entirely. Pre-existing bug, NOT introduced by Phase 2. **Resolution path:** P-LIFECYCLE's resolve-at-insert hook eliminates the only path that reaches the failing branch by ensuring new MLS rows arrive with `assigned_agent_id` populated. A standalone RLS fix on `tenant_floor_pool` is also viable as a defensive patch if NULL-cache rows persist after P-LIFECYCLE ships.
