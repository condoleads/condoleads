...
**Status:** Tracker v23 (2026-05-30). Model LOCKED (v16, 2026-05-28). Phase 1 + Phase 2 + P-LIFECYCLE Landing 1 SHIPPED; P-LIFECYCLE Landing 2 SHIPPED (primitive `2ca82c6` + sync hooks `433c553`); Landing 3 (Events 4 + 7) remains. This document is the authoritative spec for territory + lead routing.
...
- **F-NIGHTLY-SYNC-TIMEOUT-6H (OPEN, operational, separate ticket)** — Surfaced in Landing 2 verify recon via `gh run list --workflow=nightly-sync.yml --limit 8`: ALL 8 most recent runs (2026-05-21 through 2026-05-29) cancelled at exactly ~6h runtime — GH Actions' default 360-min job timeout. `nightly-sync.yml` does not set `timeout-minutes`, so the cap is the default 6h. The sync is hitting that cap every night and not completing. This is a pre-existing operational issue, NOT introduced by any routing work. **Implication for Landing 2:** the next-morning NULL-cache drop acceptance criterion (Landing 2 plan section 11) is BLOCKED on this; the Landing 2 primitive is correct in isolation but cannot be validated end-to-end via the sync until the sync itself completes within budget. Fix shape (separate ticket): split the workflow into smaller jobs OR increase `timeout-minutes` OR move long-running work to a workspace runner OR shard by municipality. Not a Landing 2 fix to make.
