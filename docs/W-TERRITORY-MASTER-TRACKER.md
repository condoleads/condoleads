...
**Status:** Tracker v22 (2026-05-29). Model LOCKED (v16, 2026-05-28). Phase 1 + Phase 2 + P-LIFECYCLE Landing 1 SHIPPED; P-LIFECYCLE Landing 2 primitive SHIPPED (`2ca82c6`); Landing 2 sync hooks + Landing 3 remain. This document is the authoritative spec for territory + lead routing.
...
| **P-LIFECYCLE** | **Wire the 7 lifecycle events (insert-resolve, departure-reflow, geo-change, reconcile, etc.)** | **IN PROGRESS** — Landing 1 SHIPPED 2026-05-29 (`4e69773`, v21 tracker `d1d7d4f`); Landing 2 primitive SHIPPED 2026-05-29 (`2ca82c6`); Landing 2 sync hooks + Landing 3 (deactivation reflow + nightly reconcile) REMAIN |
...
### P-LIFECYCLE — IN PROGRESS (Landing 1 + Landing 2 primitive SHIPPED 2026-05-29)
...
- **Landing 2 — Event 5 + Event 6: PRIMITIVE SHIPPED 2026-05-29, commit `2ca82c6`. Sync hooks remain.**
  ...
  - **REMAINING (separate work):** TypeScript sync hooks in `lib/homes-sync/save.ts` + `lib/building-sync/save.ts` calling `reresolve_listings_in_set` after upsert; geo-diff utility in new file `lib/utils/geo-diff.ts`; `WALLIAM_TENANT_ID` constant in new file `scripts/lib/territory-constants.ts`. Acceptance for next-morning NULL-cache drop is BLOCKED-ON-F-NIGHTLY-SYNC-TIMEOUT-6H per plan doc section 11 (sync currently cancels at GH Actions' 360-min cap every night; ops fix is a separate ticket).
...
- **F-RESOLVE-AT-INSERT-PRIORITY (PARTIALLY RESOLVED 2026-05-29 at primitive layer, Landing 2 `2ca82c6`; sync hook step remains BLOCKED-ON-F-NIGHTLY-SYNC-TIMEOUT-6H)** — 34 NULL condo/home rows accumulated in 1h38m between Phase 1 COMMIT (2026-05-29 10:04 UTC) and the post-COMMIT verification probe. Cause: no resolve-at-insert hook — `assigned_agent_id` was left NULL on every new MLS row inserted by nightly sync (verified in P-MODEL-recon Q5: `lib/homes-sync/save.ts` does not write the column [path corrected from v21's "`lib/homes-save.ts`"]; no insert trigger fills it). Confirmed decision-A (resolve-at-insert) from the v16 model as the active drift source producing daily NULL accumulation. **Primitive-layer RESOLUTION (Landing 2):** `reresolve_listings_in_set(uuid[], uuid)` is the set-based resolver the sync will call after each upsert batch + on geo-diff. V1-V6 in-tx + T1-T5 smoke prove the primitive works correctly and produces walk-equivalent results modulo N=1 carves. **REMAINING (separate work):** TypeScript hooks in `lib/homes-sync/save.ts` + `lib/building-sync/save.ts` + new `lib/utils/geo-diff.ts` + new `scripts/lib/territory-constants.ts`. End-to-end next-morning NULL-cache drop acceptance is BLOCKED-ON-F-NIGHTLY-SYNC-TIMEOUT-6H per Landing 2 plan section 11 (the nightly sync is already cancelling at GH Actions' 360-min cap every night; ops fix is a separate ticket).
