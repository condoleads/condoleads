 ...
 */
export async function findComparables(specs: UnitSpecs, customValues?: AdjustmentValues): Promise<MatchResult> {
  ...
  const bingoMatches = findBingoMatches(bedBathMatches, specs, false, customValues)
  ...
  const bingoAdjMatches = findBingoMatches(bedBathMatches, specs, true, customValues)
  ...
  const rangeMatches = findRangeMatches(bedBathMatches, specs, false, customValues)
  if (rangeMatches.length > 0) {
    return { tier: 'RANGE', comparables: rangeMatches }
  }

  // TIER 4: RANGE-ADJ - Same sqft range + parking/locker may differ
  const rangeAdjMatches = findRangeMatches(bedBathMatches, specs, true, customValues)
  ...
  const maintMatches = findMaintenanceMatches(bedBathMatches, specs, false, customValues)
  ...
  const maintAdjMatches = findMaintenanceMatches(bedBathMatches, specs, true, customValues)
 ...
 */
function findBingoMatches(sales: any[], specs: UnitSpecs, allowAdjustments: boolean, customValues?: AdjustmentValues): ComparableSale[] {
  ...
  return matches.map(sale => createComparable(sale, specs, allowAdjustments, customValues))
}

/**
 * RANGE Match: Same sqft range + bed + bath + (parking + locker if not adjusted)
 */
function findRangeMatches(sales: any[], specs: UnitSpecs, allowAdjustments: boolean, customValues?: AdjustmentValues): ComparableSale[] {
  const matches = sales.filter(sale => {
 ...
 */
function findMaintenanceMatches(sales: any[], specs: UnitSpecs, allowAdjustments: boolean, customValues?: AdjustmentValues): ComparableSale[] {
 ...
 */
function createComparable(sale: any, specs: UnitSpecs, applyAdjustments: boolean, customValues?: AdjustmentValues): ComparableSale {
  ...
  if (applyAdjustments) {
    ...
    if (parkingDiff !== 0) {
      const adjustmentAmount = parkingDiff * (customValues?.parkingPerSpace ?? ADJUSTMENT_VALUES.PARKING_PER_SPACE)
    ...
    if (userHasLocker !== compHasLocker) {
      const adjustmentAmount = userHasLocker ? (customValues?.locker ?? ADJUSTMENT_VALUES.LOCKER) : -(customValues?.locker ?? ADJUSTMENT_VALUES.LOCKER)
      adjustedPrice += adjustmentAmount
      adjustments.push({
        type: 'locker',
