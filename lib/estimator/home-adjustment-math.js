...
// v10 step 3 Phase 1.1 (2026-06-09): optional customValues param. Reads use
// `customValues?.X ?? adj.X` so a caller that doesn't pass customValues (e.g.
// the backtest, or any score-only call site) gets DEFAULT_ADJUSTMENTS verbatim
// — byte-identical to pre-Phase-1.1 behavior. Only an explicitly-set per-
// tenant override row alters the basement total.
function getBasementAdjustment(subjectArr, compArr, customValues) {
  const subject = parseBasement(subjectArr)
  const comp = parseBasement(compArr)
  const adj = DEFAULT_ADJUSTMENTS
  const FINISHED = customValues?.BASEMENT_FINISHED ?? adj.BASEMENT_FINISHED
  const SEP = customValues?.BASEMENT_SEP_ENTRANCE ?? adj.BASEMENT_SEP_ENTRANCE
  const WALKOUT = customValues?.BASEMENT_WALKOUT_BONUS ?? adj.BASEMENT_WALKOUT_BONUS
...
// v10 step 3 Phase 1.1 (2026-06-09): optional customValues param. Same
// `??` fallback pattern — a caller passing no customValues (or where the
// scoring path deliberately keeps DEFAULT — see scoreMatch) gets identical
// behavior to pre-Phase-1.1. The 'Carport' = $15K (half of detached) value
// is a derived constant, not a DEFAULT_ADJUSTMENTS key, so it stays
// hardcoded; GARAGE_ATTACHED_DOUBLE has no MLS-side type that maps to it
// today (forward-compat only — table column carries it but matcher doesn't
// branch on 'Double Attached').
function getGarageValue(garageType, customValues) {
  const adj = DEFAULT_ADJUSTMENTS
  switch (garageType) {
    case 'Detached': return customValues?.GARAGE_DETACHED_SINGLE ?? adj.GARAGE_DETACHED_SINGLE
    case 'Attached': return customValues?.GARAGE_ATTACHED_SINGLE ?? adj.GARAGE_ATTACHED_SINGLE
    case 'Built-In': return customValues?.GARAGE_BUILTIN ?? adj.GARAGE_BUILTIN
