...
interface AdjustmentValuesLease {
 ...
 */
export async function findComparablesRentals(specs: UnitSpecs, customValues?: AdjustmentValuesLease): Promise<MatchResult> {
 ...
 */
function findBingoMatches(leases: any[], specs: UnitSpecs, allowAdjustments: boolean, customValues?: AdjustmentValuesLease): ComparableSale[] {
  ...
  return matches.map(lease => createComparable(lease, specs, allowAdjustments, customValues))
}

/**
 * RANGE Match: Same sqft range + bed + bath + (parking + locker if not adjusted)
 */
function findRangeMatches(leases: any[], specs: UnitSpecs, allowAdjustments: boolean, customValues?: AdjustmentValuesLease): ComparableSale[] {
  const matches = leases.filter(lease => {
 ...
 */
function createComparable(lease: any, specs: UnitSpecs, applyAdjustments: boolean, customValues?: AdjustmentValuesLease): ComparableSale {
  ...
  if (applyAdjustments) {
    ...
    if (parkingDiff !== 0) {
      const adjustmentAmount = parkingDiff * (customValues?.parkingPerSpace ?? ADJUSTMENT_VALUES_LEASE.PARKING_PER_SPACE)
    ...
    if (userHasLocker !== compHasLocker) {
      const adjustmentAmount = userHasLocker ? (customValues?.locker ?? ADJUSTMENT_VALUES_LEASE.LOCKER) : -(customValues?.locker ?? ADJUSTMENT_VALUES_LEASE.LOCKER)
      adjustedPrice += adjustmentAmount
      adjustments.push({
        type: 'locker',
