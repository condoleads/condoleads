...
// Format activity type for email subject
function formatActivityType(activityType: string): string {
  const typeMap: Record<string, string> = {
    ...
    'lease_evaluation_request': 'Lease Evaluation Request',
    'building_visit_request': 'Building Visit Request',
    'estimator_used': 'Used Price Estimator',
    'estimator_contact_submitted': 'Estimator Contact Submitted',
...
export async function sendActivityEmail({
  ...
  try {
    ...
    const buildingName = lead.buildings?.name || latestActivity.activity_data?.buildingName
