...
// =====================================================
// HELPER PARSING FUNCTIONS (exact copy from original)
// =====================================================
...
export async function saveBuilding(buildingData: BuildingData, listingsData: any[]): Promise<SaveResult> {
  console.log(`[DirectSave] Starting: ${buildingData.buildingName} (${listingsData.length} listings)`);
  const startTime = Date.now();

  try {
    // STEP 1: Clean existing data
    await cleanExistingBuilding(buildingData.streetNumber, buildingData.streetName, buildingData.city);

    // STEP 2: Insert building
    const building = await insertBuilding(buildingData, listingsData.length);
    console.log(`[DirectSave] Building created: ${building.id}`);

    // STEP 3: Save listings with COMPLETE DLA mapping
    const savedListings = await saveListings(building.id, listingsData);
    console.log(`[DirectSave] Listings saved: ${savedListings.length}`);

    // STEP 4: Save media
    const mediaCount = await saveMedia(savedListings, listingsData);
    console.log(`[DirectSave] Media saved: ${mediaCount}`);

    // STEP 5: Save rooms
    const roomCount = await saveRooms(savedListings, listingsData);
    console.log(`[DirectSave] Rooms saved: ${roomCount}`);

    // STEP 6: Save open houses
    const openHouseCount = await saveOpenHouses(savedListings, listingsData);
    console.log(`[DirectSave] Open houses saved: ${openHouseCount}`);
    ...
    console.log(`[DirectSave] Complete in ${duration.toFixed(1)}s`);
    ...
    console.error('[DirectSave] Error:', error.message);
    return { success: false, error: error.message };
  }
}

// =====================================================
// STEP 1: CLEAN EXISTING BUILDING
// =====================================================

async function cleanExistingBuilding(streetNumber: string, streetName: string, city: string) {
  const streetFirstWord = streetName.split(' ')[0];
  console.log(`[DirectSave] Cleaning: ${streetNumber} ${streetFirstWord}%, ${city}`);

  const { data: existingBuildings } = await supabase
    .from('buildings')
    .select('id, building_name')
    .eq('street_number', streetNumber)
    .ilike('street_name', `${streetFirstWord}%`)
  ...
  if (existingBuildings && existingBuildings.length > 0) {
    for (const building of existingBuildings) {
      ...
      if (listingIds.length > 0) {
        ...
        await supabase.from('mls_listings').delete().eq('building_id', building.id);
      }

      await supabase.from('sync_history').delete().eq('building_id', building.id);
      await supabase.from('buildings').delete().eq('id', building.id);
      console.log(`[DirectSave] Deleted: ${building.building_name}`);
    }
  }
}
...
async function insertBuilding(buildingData: BuildingData, totalListings: number) {
  const { data, error } = await supabase
    .from('buildings')
    .insert({
      slug: buildingData.slug,
      building_name: buildingData.buildingName,
      canonical_address: buildingData.canonicalAddress,
      street_number: buildingData.streetNumber,
      street_name: buildingData.streetName,
      city_district: buildingData.city || 'Toronto',
      total_units: totalListings,
...
async function saveListings(buildingId: string, listingsData: any[]) {
...
async function saveMedia(savedListings: any[], listingsData: any[]) {
  ...
  if (mediaRecords.length > 0) {
    ...
    for (let i = 0; i < mediaRecords.length; i += batchSize) {
      ...
      if (error) console.error('[DirectSave] Media error:', error.message);
...
async function saveRooms(savedListings: any[], listingsData: any[]) {
  ...
  if (roomRecords.length > 0) {
    ...
    for (let i = 0; i < roomRecords.length; i += batchSize) {
      ...
      if (error) console.error('[DirectSave] Rooms error:', error.message);
      else totalCount += batch.length;
    }
  }
...
async function saveOpenHouses(savedListings: any[], listingsData: any[]) {
  ...
  if (openHouseRecords.length > 0) {
    ...
    for (let i = 0; i < openHouseRecords.length; i += batchSize) {
      ...
      if (error) console.error('[DirectSave] Open houses error:', error.message);
      else totalCount += batch.length;
    }
  }
...
// =====================================================
// STEP 7: GEOGRAPHIC HIERARCHY
// =====================================================

async function ensureGeographicHierarchy(
  areaName: string | null,
  municipalityName: string | null,
  communityName: string | null
): Promise<string | null> {
  if (!communityName || !municipalityName) return null;
  ...
  try {
    ...
    if (!existingArea) {
      const { data: newArea, error: areaError } = await supabase
        .from('treb_areas')
        .insert({ name: area, code: area.substring(0, 20) })
        .select('id')
        .single();
    ...
    // STEP 2: Ensure Municipality exists
    let { data: existingMunicipality } = await supabase
      .from('municipalities')
      .select('id')
      .eq('area_id', areaId)
    ...
    if (!existingMunicipality) {
      ...
      const { data: newMunicipality, error: munError } = await supabase
        .from('municipalities')
        .insert({ area_id: areaId, name: municipalityName, code: code })
        .select('id')
        .single();
    ...
    // STEP 3: Ensure Community exists
    let { data: existingCommunity } = await supabase
      .from('communities')
      .select('id')
    ...
    if (!existingCommunity) {
      const { data: newCommunity, error: commError } = await supabase
        .from('communities')
        .insert({ municipality_id: municipalityId, name: communityName })
        .select('id')
        .single();
    ...
    console.error('[DirectSave] Hierarchy error:', error.message);
    return null;
  }
}

async function linkBuildingToHierarchy(buildingId: string, listingsData: any[]): Promise<void> {
  const firstListing = listingsData[0];
  if (!firstListing) return;

  const communityId = await ensureGeographicHierarchy(
    firstListing.CountyOrParish,
    firstListing.City,
    firstListing.CityRegion
  );
...
async function createSyncHistory(
  ...
  await supabase.from('sync_history').insert({
    ...
    listings_created: totalSaved,
    media_records_created: mediaCount,
    room_records_created: roomCount,
    open_house_records_created: openHouseCount,
    sync_status: 'success',
